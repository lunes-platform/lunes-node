...
import io.lunes.database.patch.DisableHijackedAliases
...
import io.lunes.transaction.Transaction.Type
import io.lunes.transaction.ValidationError.{AliasDoesNotExist, AliasIsDisabled}
...
object LevelDBWriter {
  ...
  /** {{{([15, 12, 3], [12, 5]) => [(15, 12), (12, 12), (3, 12), (3, 5)]}}}
    *
    * @param wbh LUNES balance history
...
class LevelDBWriter(writableDB: DB,
  ...
  override protected def loadLastBlock(): Option[Block] =
    readOnly(db => db.get(Keys.blockAt(db.get(Keys.height))))
  ...
  override def balance(address: Address, mayBeAssetId: Option[AssetId]): Long =
    readOnly { db =>
      addressId(address).fold(0L) { addressId =>
        mayBeAssetId match {
          case Some(assetId) =>
            db.fromHistory(Keys.assetBalanceHistory(addressId, assetId),
                           Keys.assetBalance(addressId, assetId))
              .getOrElse(0L)
          case None =>
            db.fromHistory(Keys.lunesBalanceHistory(addressId),
                           Keys.lunesBalance(addressId))
              .getOrElse(0L)
        }
      }
    }
  ...
  private def loadLposPortfolio(db: ReadOnlyDB, addressId: BigInt) = Portfolio(
    db.fromHistory(Keys.lunesBalanceHistory(addressId),
                   Keys.lunesBalance(addressId))
      .getOrElse(0L),
    db.fromHistory(Keys.leaseBalanceHistory(addressId),
  ...
  private def loadPortfolio(db: ReadOnlyDB, addressId: BigInt) =
    loadLposPortfolio(db, addressId).copy(
      assets = (for {
        ...
        assetId -> db
          .fromHistory(Keys.assetBalanceHistory(addressId, assetId),
                       Keys.assetBalance(addressId, assetId))
          .getOrElse(0L)).toMap
    )
  ...
  override protected def loadAssetDescription(
      assetId: ByteStr): Option[AssetDescription] = readOnly { db =>
    db.get(Keys.transactionInfo(assetId)) match {
      case Some((_, i: IssueTransaction)) =>
        val ai = db
          .fromHistory(Keys.assetInfoHistory(assetId), Keys.assetInfo(assetId))
          .getOrElse(AssetInfo(i.reissuable, i.quantity, i.script))
        val sponsorship = db
          .fromHistory(Keys.sponsorshipHistory(assetId),
                       Keys.sponsorship(assetId))
          .fold(0L)(_.minFee)
        Some(
  ...
  override protected def loadActivatedFeatures(): Map[Short, Int] =
    fs.preActivatedFeatures ++ readOnly(_.get(Keys.activatedFeatures))
  ...
  override protected def doAppend(
    ...
    val newAddressesForLunes = ArrayBuffer.empty[BigInt]
    for ((addressId, balance) <- lunesBalances) {
      val kwbh = Keys.lunesBalanceHistory(addressId)
      val wbh = rw.get(kwbh)
      if (wbh.isEmpty) {
        newAddressesForLunes += addressId
      }
      rw.put(Keys.lunesBalance(addressId)(height), balance)
    ...
    if (newAddressesForLunes.nonEmpty) {
      val newSeqNr = rw.get(Keys.addressesForLunesSeqNr) + 1
      rw.put(Keys.addressesForLunesSeqNr, newSeqNr)
      rw.put(Keys.addressesForLunes(newSeqNr), newAddressesForLunes)
    }
    ...
    for ((addressId, assets) <- assetBalances) {
      ...
      for (assetId <- newAssets) {
        newAddressesForAsset += assetId -> (newAddressesForAsset
          .getOrElse(assetId, Set.empty) + addressId)
      }
    ...
    for ((assetId, assetInfo) <- reissuedAssets) {
      ...
      expiredKeys ++= updateHistory(rw,
                                    Keys.assetInfoHistory(assetId),
                                    threshold,
                                    Keys.assetInfo(assetId))
    }
    ...
    val accountTransactions = (for {
      ...
      addressId <- addresses
    } yield (addressId, (tx.builder.typeId.toInt, id)))
      .groupBy(_._1)
    ...
    for ((assetId, sp: SponsorshipValue) <- sponsorship) {
      ...
      expiredKeys ++= updateHistory(rw,
                                    Keys.sponsorshipHistory(assetId),
                                    threshold,
                                    Keys.sponsorship(assetId))
    }
        ...
        for (currentHeight <- height until targetHeight by -1) {
          ...
          readWrite { rw =>
            ...
            for (addressId <- rw.get(Keys.changedAddresses(currentHeight))) {
              ...
              rw.delete(Keys.lunesBalance(addressId)(currentHeight))
              rw.filterHistory(Keys.lunesBalanceHistory(addressId),
                               currentHeight)
            ...
            for (txId <- rw.get(txIdsAtHeight)) {
              ...
              for ((_, tx) <- rw.get(ktxId)) tx match {
                ...
                case _: PaymentTransaction | _: TransferTransaction |
                    _: MassTransferTransaction => // balances already restored

                case _: IssueTransaction =>
                ...
                case tx: SponsorFeeTransaction =>
                  assetInfoToInvalidate += rollbackSponsorship(rw,
                                                               tx.assetId,
                                                               currentHeight)
                case _: LeaseTransaction =>
                ...
                case tx: DataTransaction =>
                  ...
                  addressId(address).foreach { addressId =>
                    tx.data.foreach { e =>
                      log.trace(
                        s"Discarding ${e.key} for $address at $currentHeight")
                      rw.delete(Keys.data(addressId, e.key)(currentHeight))
                      rw.filterHistory(Keys.dataHistory(addressId, e.key),
                                       currentHeight)
                    }
                  }
                ...
                case tx: ExchangeTransaction =>
                  ordersToInvalidate += rollbackOrderFill(
                    rw,
                    ByteStr(tx.buyOrder.id()),
                    currentHeight)
                  ordersToInvalidate += rollbackOrderFill(
                    rw,
                    ByteStr(tx.sellOrder.id()),
                    currentHeight)
              }
  ...
  override def transactionInfo(id: ByteStr): Option[(Int, Transaction)] =
    readOnly(db => db.get(Keys.transactionInfo(id)))

  override def transactionHeight(id: ByteStr): Option[Int] =
    readOnly(db => db.get(Keys.transactionHeight(id)))
    ...
    readOnly { db =>
      db.get(Keys.addressId(address)).fold(Seq.empty[(Int, Transaction)]) {
        addressId =>
          val txs = for {
            seqNr <- (db
              .get(Keys.addressTransactionSeqNr(addressId)) to 1 by -1).view
            (txType, txId) <- db.get(
              Keys.addressTransactionIds(addressId, seqNr))
  ...
  override def collectLposPortfolios[A](
      pf: PartialFunction[(Address, Portfolio), A]) = readOnly { db =>
  ...
  override def blockHeaderAndSize(
    ...
    readOnly(db =>
      db.get(Keys.heightOf(blockId)).flatMap(h => db.get(Keys.blockHeader(h))))
  ...
  override def blockBytes(blockId: ByteStr): Option[Array[Byte]] =
    readOnly(db =>
      db.get(Keys.heightOf(blockId)).flatMap(h => db.get(Keys.blockBytes(h))))
  ...
  override def parent(block: Block, back: Int): Option[Block] = readOnly { db =>
    db.get(Keys.heightOf(block.reference))
      .flatMap(h => db.get(Keys.blockAt(h - back + 1)))
  }
  ...
  override def featureVotes(height: Int): Map[Short, Int] = readOnly { db =>
    fs.activationWindow(height)
      .flatMap(
        h =>
          db.get(Keys.blockHeader(h))
            .fold(Seq.empty[Short])(_._1.featureVotes.toSeq))
      .groupBy(identity)
      .mapValues(_.size)
  }
  ...
  override def assetDistribution(assetId: ByteStr): Map[Address, Long] =
    readOnly { db =>
      (for {
        ...
        balance <- db.fromHistory(Keys.assetBalanceHistory(addressId, assetId),
                                  Keys.assetBalance(addressId, assetId))
        if balance > 0
      } yield db.get(Keys.idToAddress(addressId)) -> balance).toMap.seq
    }

  override def lunesDistribution(height: Int): Map[Address, Long] = readOnly {
    db =>
      (for {
