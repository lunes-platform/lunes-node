...
import io.lunes.state.{ByteStr, EitherExt2}
...
trait Codec[A] {
  ...
  protected def decodeShort(bytes: Array[Byte]): Either[CodecFailure, Short] =
    Try(Shorts.fromByteArray(bytes)).toEither.left.map(e =>
      CodecFailure(e.getMessage))

  protected def decodeInt(bytes: Array[Byte]): Either[CodecFailure, Int] =
    Try(Ints.fromByteArray(bytes)).toEither.left.map(e =>
      CodecFailure(e.getMessage))

  protected def decodeLong(bytes: Array[Byte]): Either[CodecFailure, Long] =
    Try(Longs.fromByteArray(bytes)).toEither.left.map(e =>
      CodecFailure(e.getMessage))

  protected def encodeBoolean(value: Boolean): Array[Byte] =
    if (value) TrueBytes else FalseBytes

  protected def decodeBoolean(
      bytes: Array[Byte]): Either[CodecFailure, Boolean] =
    Try(bytes.take(1).sameElements(TrueBytes)).toEither.left.map(e =>
      CodecFailure(e.getMessage))
...
object BlockCheckpointCodec extends Codec[BlockCheckpoint] {
  override def encode(bcp: BlockCheckpoint): Array[Byte] = {
    ...
    System.arraycopy(bcp.signature,
                     0,
                     result,
                     Ints.BYTES,
                     Codec.SignatureLength)
    result
  }

  override def decode(
      arr: Array[Byte]): Either[CodecFailure, DecodeResult[BlockCheckpoint]] = {
    val len = Ints.BYTES + Codec.SignatureLength
    for {
      height <- Try(Ints.fromByteArray(arr.take(Ints.BYTES))).toEither.left
        .map(e => CodecFailure(e.getMessage))
      signature <- Either.cond(arr.length >= len,
                               arr.slice(Ints.BYTES, len),
                               CodecFailure("not enough bytes for signature"))
...
object CheckpointCodec extends Codec[Checkpoint] {
  ...
  override def decode(
      bytes: Array[Byte]): Either[CodecFailure, DecodeResult[Checkpoint]] = {
    val signature = bytes.take(Codec.SignatureLength)
    for {
      _ <- Either.cond(signature.length == Codec.SignatureLength,
                       (),
                       CodecFailure("incorrect signature length"))
      items <- itemsCodec.decode(
        bytes.slice(Codec.SignatureLength, bytes.length))
    } yield
      DecodeResult(Codec.SignatureLength + items.length,
                   Checkpoint(items.value, signature))
  }
}

case class Tuple2Codec[A, B](aCodec: Codec[A], bCodec: Codec[B])
    extends Codec[(A, B)] {
  ...
  override def decode(
      bytes: Array[Byte]): Either[CodecFailure, DecodeResult[(A, B)]] =
    for {
...
case class OptionCodec[A](valueCodec: Codec[A]) extends Codec[Option[A]] {
  ...
  override def decode(
      bytes: Array[Byte]): Either[CodecFailure, DecodeResult[Option[A]]] = {
...
object StringCodec extends Codec[String] {
  ...
  override def decode(
      bytes: Array[Byte]): Either[CodecFailure, DecodeResult[String]] = {
    for {
      ...
      _ <- Either.cond(
        a.length == s,
        (),
        CodecFailure("incorrect string byte representation length"))
      v <- Try(new String(a, Charsets.UTF_8)).toEither.left.map(e =>
        CodecFailure(e.getMessage))
...
object StringSeqCodec extends Codec[Seq[String]] {
  ...
  override def encode(value: Seq[String]): Array[Byte] =
    itemsCodec.encode(value)

  override def decode(
      bytes: Array[Byte]): Either[CodecFailure, DecodeResult[Seq[String]]] =
...
object OrderToTxIdsCodec extends Codec[Set[String]] {
  ...
  override def encode(value: Set[String]): Array[Byte] =
    itemsCodec.encode(value.toSeq)

  override def decode(
      bytes: Array[Byte]): Either[CodecFailure, DecodeResult[Set[String]]] =
    itemsCodec
      .decode(bytes)
      .right
      .map(r => DecodeResult(r.length, r.value.toSet))
}

object OrderIdsCodec extends Codec[Array[String]] {
  private val itemsCodec = SeqCodec(StringCodec)

  override def encode(value: Array[String]): Array[Byte] =
    itemsCodec.encode(value.toSeq)

  override def decode(
      bytes: Array[Byte]): Either[CodecFailure, DecodeResult[Array[String]]] =
    itemsCodec
      .decode(bytes)
      .right
      .map(r => DecodeResult(r.length, r.value.toArray))
}

object AssetIdOrderIdCodec
    extends Tuple2Codec(OptionCodec[AssetId](ByteStrCodec), StringCodec)

object AssetIdOrderIdSetCodec
    extends ColCodec[Set, (Option[AssetId], String)](AssetIdOrderIdCodec)
...
object PortfolioItemCodec extends Codec[(String, Long)] {
  ...
  override def decode(bytes: Array[Byte])
    : Either[CodecFailure, DecodeResult[(String, Long)]] = {
    for {
...
object PortfolioCodec extends Codec[Map[String, Long]] {
  ...
  override def encode(value: Map[String, Long]): Array[Byte] =
    itemsCodec.encode(value.toSeq)

  override def decode(bytes: Array[Byte])
    : Either[CodecFailure, DecodeResult[Map[String, Long]]] =
    itemsCodec
      .decode(bytes)
      .right
...
object ByteStrCodec extends Codec[ByteStr] {
  ...
  override def decode(
      bytes: Array[Byte]): Either[CodecFailure, DecodeResult[ByteStr]] = {
    for {
      l <- decodeInt(bytes.take(Ints.BYTES))
      a = bytes.slice(Ints.BYTES, Ints.BYTES + l)
      _ <- Either.cond(a.length == l,
                       (),
                       CodecFailure("incorrect ByteStr length"))
...
object ShortCodec extends Codec[Short] {
  ...
  override def decode(
      bytes: Array[Byte]): Either[CodecFailure, DecodeResult[Short]] = {
    for {
      v <- Try(Shorts.fromByteArray(bytes.take(Shorts.BYTES))).toEither.left
        .map(e => CodecFailure(e.getMessage))
...
object ShortSeqCodec extends Codec[Seq[Short]] {
  ...
  override def decode(
      bytes: Array[Byte]): Either[CodecFailure, DecodeResult[Seq[Short]]] =
    itemsCodec.decode(bytes)
}
...
object KeyCodec extends Codec[ByteStr] {
  ...
  override def decode(
    ...
    Either.cond(a.length == KeySize,
                DecodeResult(KeySize, ByteStr(a)),
                CodecFailure("incorrect key size"))
  }
}
...
object KeySeqCodec extends Codec[Seq[ByteStr]] {
  ...
  override def encode(value: Seq[ByteStr]): Array[Byte] =
    itemsCodec.encode(value)

  override def decode(
      bytes: Array[Byte]): Either[CodecFailure, DecodeResult[Seq[ByteStr]]] =
    itemsCodec.decode(bytes)
}
...
object Id32Codec extends Codec[ByteStr] {
  ...
  override def decode(
    ...
    Either.cond(a.length == IdSize,
                DecodeResult(IdSize, ByteStr(a)),
                CodecFailure("incorrect id32 size"))
  }
}
