...
object ScriptEstimator {
  def apply(functionCosts: Map[FunctionHeader, Long],
            t: EXPR): Either[String, Long] = {
    type Result[T] = EitherT[Coeval, String, T]

    def aux(t: Result[EXPR],
            syms: Map[String, EXPR]): Result[(Long, Map[String, EXPR])] =
      t.flatMap {
        case _: CONST_LONG | _: CONST_BYTEVECTOR | _: CONST_STRING | TRUE |
            FALSE =>
          EitherT.pure((1, syms))
        case t: GETTER =>
          aux(EitherT.pure(t.expr), syms).map {
            case (comp, out) => (comp + 2, out)
          }
        ...
        case t: FUNCTION_CALL =>
          for {
            callCost <- EitherT.fromOption[Coeval](
              functionCosts.get(t.function),
              s"Unknown function ${t.function}")
