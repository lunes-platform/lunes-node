...
case class EvaluationContext(letDefs: Map[String, LazyVal],
                             functions: Map[FunctionHeader, PredefFunction])

object EvaluationContext {

  object Lenses {
    val lets: Lens[EvaluationContext, Map[String, LazyVal]] = lens[
      EvaluationContext] >> 'letDefs
    val funcs
      : Lens[EvaluationContext, Map[FunctionHeader, PredefFunction]] = lens[
      EvaluationContext] >> 'functions
  }

  val empty = EvaluationContext(Map.empty, Map.empty)

  implicit val monoid: Monoid[EvaluationContext] =
    new Monoid[EvaluationContext] {
      override val empty: EvaluationContext = EvaluationContext.empty

      override def combine(x: EvaluationContext,
                           y: EvaluationContext): EvaluationContext =
        EvaluationContext(letDefs = x.letDefs ++ y.letDefs,
                          functions = x.functions ++ y.functions)
    }

  def build(letDefs: Map[String, LazyVal],
    ...
    EvaluationContext(letDefs = letDefs,
