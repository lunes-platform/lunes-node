...
import io.lunes.lang.v1.evaluator.ctx.{
  CaseObj,
  EvaluationContext,
  LazyVal,
  PredefFunction
...
object PureContext {
  ...
  private val noneCoeval: Coeval[Either[String, Option[Nothing]]] =
    Coeval.evalOnce(Right(None))
  private val nothingCoeval: Coeval[Either[String, Nothing]] = Coeval.defer(
    Coeval(Right(throw new Exception("explicit contract termination"))))

  val none: LazyVal = LazyVal(
    EitherT(noneCoeval).subflatMap(Right(_: Option[Nothing]))) // IDEA HACK
  val err = LazyVal(EitherT(nothingCoeval))
  val errRef = "throw"

  val fraction: PredefFunction = PredefFunction(
    "fraction",
    1,
    LONG,
    List(("value", LONG), ("numerator", LONG), ("denominator", LONG)),
    FRACTION) {
    case (v: Long) :: (n: Long) :: (d: Long) :: Nil => {
      val result = BigInt(v) * n / d
      for {
        _ <- Either.cond(result < Long.MaxValue,
                         (),
                         s"Long overflow: value `$result` greater than 2^63-1")
        _ <- Either.cond(result > Long.MinValue,
                         (),
                         s"Long overflow: value `$result` less than -2^63-1")
      } yield result.toLong
    }
    case _ => ???
  }

  val extract: PredefFunction = PredefFunction("extract",
                                               1,
                                               TYPEPARAM('T'),
                                               List(("opt", optionT)),
                                               EXTRACT) {
  ...
  val some: PredefFunction =
    PredefFunction("Some", 1, optionT, List(("obj", TYPEPARAM('T'))), SOME) {
      case v :: Nil => Right(Some(v))
      case _        => ???
    }

  val _isInstanceOf: PredefFunction =
    PredefFunction("_isInstanceOf",
                   1,
                   BOOLEAN,
                   List(("obj", TYPEPARAM('T')), ("of", STRING)),
                   ISINSTANCEOF) {
      case (p: CaseObj) :: (s: String) :: Nil => Right(p.caseType.name == s)
      case _                                  => ???
    }

  val isDefined: PredefFunction =
    PredefFunction("isDefined", 1, BOOLEAN, List(("opt", optionT)), ISDEFINED) {
      case Some(_) :: Nil => Right(true)
      case None :: Nil    => Right(false)
      case _              => ???
    }

  val size: PredefFunction = PredefFunction("size",
                                            1,
                                            LONG,
  ...
  private def createOp(op: BinaryOperation, t: TYPE, r: TYPE, func: Short)(
      body: (t.Underlying, t.Underlying) => r.Underlying) = {
  ...
  val getElement =
    PredefFunction("getElement",
                   2,
                   TYPEPARAM('T'),
                   List("arr" -> LISTTYPEPARAM(TYPEPARAM('T')), "pos" -> LONG),
                   GET_LIST) {
      case (arr: IndexedSeq[_]) :: (pos: Long) :: Nil =>
        Try(arr(pos.toInt)).toEither.left.map(_.toString)
      case _ => ???
    }

  val getListSize = PredefFunction("size",
                                   2,
                                   LONG,
                                   List("arr" -> LISTTYPEPARAM(TYPEPARAM('T'))),
                                   SIZE_LIST) {
  ...
  val uNot =
    PredefFunction("!", 1, BOOLEAN, List("p" -> BOOLEAN), NOT_BOOLEAN) {
      case (p: Boolean) :: Nil => {
        Right(!p)
      }
      case _ => ???
  ...
  private def createTryOp(op: BinaryOperation, t: TYPE, r: TYPE, func: Short)(
      body: (t.Underlying, t.Underlying) => r.Underlying) = {
    PredefFunction(opsToFunctions(op), 1, r, List("a" -> t, "b" -> t), func) {
      case a :: b :: Nil =>
        try {
          Right(
            body(a.asInstanceOf[t.Underlying], b.asInstanceOf[t.Underlying]))
        } catch {
  ...
  val sumByteVector = createOp(SUM_OP, BYTEVECTOR, BYTEVECTOR, SUM_BYTES)(
    (a, b) => ByteVector(a.toArray ++ b.toArray))
  ...
  lazy val instance =
    EvaluationContext.build(letDefs = Map(("None", none), (errRef, err)),
                            functions = Seq(fraction,
                                            extract,
                                            isDefined,
                                            some,
                                            size,
                                            _isInstanceOf) ++ operators)
