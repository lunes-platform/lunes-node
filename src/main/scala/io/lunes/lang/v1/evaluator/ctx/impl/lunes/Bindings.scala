...
object Bindings {
  ...
  private def provenTxPart(tx: Proven): Map[String, Any] =
    Map(
      ...
      "proofs" -> {
        ...
        val allProofs = existingProofs ++ Seq.fill(8 - existingProofs.size)(
          ByteVector.empty)
  ...
  private def mapRecipient(r: Recipient) =
    "recipient" -> (r match {
      case Recipient.Alias(name) =>
        CaseObj(aliasType.typeRef, Map("alias" -> name))
      case Recipient.Address(bytes) =>
        CaseObj(addressType.typeRef, Map("bytes" -> bytes))
    })
  ...
  def assetPair(ap: APair): CaseObj =
    CaseObj(
      ...
      Map(
        "amountAsset" -> ap.amountAsset
          .asInstanceOf[optionByteVector.Underlying],
  ...
  def transactionObject(tx: Tx): CaseObj =
    tx match {
      case Tx.Genesis(h, amount, recipient) =>
        CaseObj(
          genesisTransactionType.typeRef,
          Map("amount" -> amount) ++ headerPart(h) + mapRecipient(recipient))
      case Tx.Payment(p, amount, recipient) =>
        CaseObj(
          genesisTransactionType.typeRef,
          Map("amount" -> amount) ++ provenTxPart(p) + mapRecipient(recipient))
      ...
      case MassTransfer(p,
        ...
        CaseObj(
          ...
          Map(
            "transfers" -> (transfers
              .map(bv =>
                CaseObj(transfer.typeRef,
                        Map(mapRecipient(bv.recipient), "amount" -> bv.amount)))
      ...
      case SetScript(p, scriptOpt) =>
        CaseObj(
          ...
          Map(
            "script" -> scriptOpt
              .asInstanceOf[optionByteVector.Underlying]) ++ provenTxPart(p))
      case Sponsorship(p, assetId, minSponsoredAssetFee) =>
        CaseObj(
          sponsorFeeTransactionType.typeRef,
          Map("assetId" -> assetId,
              "minSponsoredAssetFee" -> minSponsoredAssetFee
                .asInstanceOf[optionLong.Underlying]) ++ provenTxPart(p)
        )
      case Data(p, data) =>
        CaseObj(
          ...
          Map(
            "data" -> data
              .map {
                case Lng(k, v) =>
                  CaseObj(longDataEntryType.typeRef,
                          Map("key" -> k, "value" -> v))
                case Str(k, v) =>
                  CaseObj(longDataEntryType.typeRef,
                          Map("key" -> k, "value" -> v))
