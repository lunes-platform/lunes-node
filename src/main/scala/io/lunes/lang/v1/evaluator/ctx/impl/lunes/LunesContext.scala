...
object LunesContext {
  ...
  def build(env: Environment): EvaluationContext = {
    ...
    def getdataF(name: String, internalName: Short, dataType: DataType) =
      PredefFunction(name,
                     100,
                     OPTION(dataType.innerType),
                     List(("address", addressType.typeRef), ("key", STRING)),
                     internalName) {
        case (addr: CaseObj) :: (k: String) :: Nil =>
          environmentFunctions.getData(addr, k, dataType)
        case _ => ???
      }

    val getLongF: PredefFunction = getdataF("getLong", DATA_LONG, DataType.Long)
    val getBooleanF: PredefFunction =
      getdataF("getBoolean", DATA_BOOLEAN, DataType.Boolean)
    val getByteArrayF: PredefFunction =
      getdataF("getByteArray", DATA_BYTES, DataType.ByteArray)
    val getStringF: PredefFunction =
      getdataF("getString", DATA_STRING, DataType.String)

    val addressFromPublicKeyF: PredefFunction =
      PredefFunction("addressFromPublicKey",
                     100,
                     addressType.typeRef,
                     List(("publicKey", BYTEVECTOR)),
                     ADDRESSFROMPUBKEY) {
    ...
    val addressFromStringF: PredefFunction =
      PredefFunction("addressFromString",
                     100,
                     optionAddress,
                     List(("string", STRING)),
                     ADDRESSFROMSTRING) {
    ...
    val addressFromRecipientF: PredefFunction =
      PredefFunction("addressFromRecipient",
                     100,
                     addressType.typeRef,
                     List(("AddressOrAlias", addressOrAliasType)),
        ...
        case c @ CaseObj(aliasType.typeRef, fields) :: Nil =>
          environmentFunctions
            ...
            .map(resolved =>
              CaseObj(addressType.typeRef, Map("bytes" -> resolved.bytes)))
        case _ => ???
      }

    val txCoeval: Coeval[Either[String, CaseObj]] =
      Coeval.evalOnce(Right(transactionObject(env.transaction)))
    val heightCoeval: Coeval[Either[String, Long]] =
      Coeval.evalOnce(Right(env.height))

    val txByIdF = {
      val returnType = OPTION(anyTransactionType)
      PredefFunction("getTransactionById",
                     100,
                     returnType,
                     List(("id", BYTEVECTOR)),
                     GETTRANSACTIONBYID) {
        case (id: ByteVector) :: Nil =>
          val maybeDomainTx =
            env.transactionById(id.toArray).map(transactionObject)
    ...
    val accountAssetBalanceF: PredefFunction =
      PredefFunction(
        ...
        100,
        LONG,
        List(("addressOrAlias", addressOrAliasType), ("assetId", BYTEVECTOR)),
        ACCOUNTASSETBALANCE) {
    ...
    val txHeightByIdF =
      PredefFunction("transactionHeightById",
                     100,
                     OPTION(LONG),
                     List(("id", BYTEVECTOR)),
                     TRANSACTIONHEIGHTBYID) {
        case (id: ByteVector) :: Nil =>
          Right(env.transactionHeightById(id.toArray))
        case _ => ???
      }

    EvaluationContext.build(
      letDefs = Map(("height", LazyVal(EitherT(heightCoeval))),
                    ("tx", LazyVal(EitherT(txCoeval)))),
      functions = Seq(
        txByIdF,
        txHeightByIdF,
        getLongF,
  ...
  var predefVars = PureContext.predefVars ++ Map(
    ("height", LONG),
    ("tx", outgoingTransactionType))
}
