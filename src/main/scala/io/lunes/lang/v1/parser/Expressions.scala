...
object Expressions {
  ...
  object PART {
    ...
    case class INVALID(start: Int, end: Int, message: String)
        extends PART[Nothing]
  }

  case class LET(start: Int,
                 end: Int,
                 name: PART[String],
                 value: EXPR,
  ...
  case class GETTER(start: Int, end: Int, ref: EXPR, field: PART[String])
      extends EXPR
  case class CONST_BYTEVECTOR(start: Int, end: Int, value: PART[ByteVector])
      extends EXPR
  case class CONST_STRING(start: Int, end: Int, value: PART[String])
      extends EXPR
  case class BINARY_OP(start: Int,
                       end: Int,
                       a: EXPR,
  ...
  case class IF(start: Int, end: Int, cond: EXPR, ifTrue: EXPR, ifFalse: EXPR)
      extends EXPR
  ...
  case class FUNCTION_CALL(start: Int,
                           end: Int,
                           name: PART[String],
                           args: List[EXPR])
      extends EXPR
  case class MATCH_CASE(start: Int,
                        end: Int,
                        newVarName: Option[PART[String]],
                        types: Seq[PART[String]],
                        expr: EXPR)
  case class MATCH(start: Int, end: Int, expr: EXPR, cases: Seq[MATCH_CASE])
      extends EXPR

  case class INVALID(start: Int,
                     end: Int,
                     message: String,
                     next: Option[EXPR] = None)
      extends EXPR
  implicit class PartOps[T](val self: PART[T]) extends AnyVal {
    def toEither: Either[String, T] = self match {
      ...
      case Expressions.PART.INVALID(s, e, message) =>
        Left(s"Can't compile an invalid instruction: $message in $s-$e")
    }
  }
