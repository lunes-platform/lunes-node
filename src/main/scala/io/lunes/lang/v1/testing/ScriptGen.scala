...
trait ScriptGen {
  ...
  def CONST_LONGgen: Gen[(EXPR, Long)] =
    Gen.choose(Long.MinValue, Long.MaxValue).map(v => (CONST_LONG(0, 0, v), v))

  def BOOLgen(gas: Int): Gen[(EXPR, Boolean)] =
    if (gas > 0)
      Gen.oneOf(GEgen(gas - 1),
                GTgen(gas - 1),
                EQ_INTgen(gas - 1),
                ANDgen(gas - 1),
                ORgen(gas - 1),
    ...
    if (gas > 0)
      Gen.oneOf(
        ...
        INTGen(gas - 1)
          ...
          .map(e =>
            (FUNCTION_CALL(0, 0, PART.VALID(0, 0, "-"), List(e._1)), -e._2))
      )
    else CONST_LONGgen
  ...
  def BLOCKgen(gas: Int): Gen[EXPR] =
    for {
      ...
      body <- Gen.oneOf(
        ...
        BLOCKgen((gas - 3) / 3)) // BLOCKGen wasn't add to BOOLGen since issue: NODE-700
  ...
  private def toString[T](part: PART[T])(implicit ct: ClassTag[T]): String =
    part match {
  ...
  def toString(expr: EXPR): Gen[String] = expr match {
    ...
    case CONST_BYTEVECTOR(_, _, x) =>
      withWhitespaces(s"""base58'${toString(x)}'""")
    ...
    case FUNCTION_CALL(_, _, PART.VALID(_, _, "-"), List(CONST_LONG(_, _, v)))
        if v >= 0 =>
      s"-($v)"
    case FUNCTION_CALL(_, _, op, List(e)) =>
      toString(e).map(e => s"${toString(op)}$e")

    case x =>
      throw new NotImplementedError(s"toString for ${x.getClass.getSimpleName}")
  }
}
