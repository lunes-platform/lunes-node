...
trait TypedScriptGen {
  ...
  def BOOLEANgen(gas: Int): Gen[EXPR] =
    if (gas > 0)
      Gen.oneOf(CONST_BOOLEANgen,
                BLOCK_BOOLEANgen(gas - 1),
                IF_BOOLEANgen(gas - 1),
                FUNCTION_CALLgen(BOOLEAN))
    else Gen.const(TRUE)
  ...
  def BLOCK_BOOLEANgen(gas: Int): Gen[EXPR] =
    for {
      let <- LETgen((gas - 3) / 3)
      body <- Gen.oneOf(BOOLEANgen((gas - 3) / 3),
                        BLOCK_BOOLEANgen((gas - 3) / 3))
  ...
  def LONGgen(gas: Int): Gen[EXPR] =
    if (gas > 0)
      Gen.oneOf(CONST_LONGgen,
                BLOCK_LONGgen(gas - 1),
                IF_LONGgen(gas - 1),
                FUNCTION_CALLgen(LONG))
    else CONST_LONGgen

  def CONST_LONGgen: Gen[EXPR] =
    Gen.choose(Long.MinValue, Long.MaxValue).map(CONST_LONG)
  ...
  def BYTESTRgen: Gen[EXPR] =
    Gen.identifier.map(x => CONST_BYTEVECTOR(ByteVector(x.getBytes)))
