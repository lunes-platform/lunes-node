...
import io.lunes.matcher.api.MatcherApiRoute
import io.lunes.matcher.market.{
  MatcherActor,
  MatcherTransactionWriter,
  OrderHistoryActor
}
import io.lunes.settings.{BlockchainSettings, RestAPISettings}
...
class Matcher(actorSystem: ActorSystem,
  ...
  lazy val orderHistory: ActorRef = actorSystem.actorOf(
    OrderHistoryActor.props(db, matcherSettings, utx, wallet),
    OrderHistoryActor.name)

  lazy val txWriter: ActorRef = actorSystem.actorOf(
    MatcherTransactionWriter.props(db, matcherSettings),
  ...
  def runMatcher() {
    ...
    val combinedRoute = CompositeHttpService(actorSystem,
                                             matcherApiTypes,
                                             matcherApiRoutes,
