...
object MatcherSettings {
  ...
  val configPath: String = "lunes.matcher"
  ...
  def fromConfig(config: Config): MatcherSettings = {
    ...
    val snapshotsInterval =
      config.as[FiniteDuration](s"$configPath.snapshots-interval")
    val orderCleanupInterval =
      config.as[FiniteDuration](s"$configPath.order-cleanup-interval")
    ...
    val basePairs: Seq[AssetPair] =
      config.getConfigList(s"$configPath.predefined-pairs").asScala.map {
        p: Config =>
          AssetPair
            .createAssetPair(p.as[String]("amountAsset"),
                             p.as[String]("priceAsset"))
            .get
      }
    val maxTimestampDiff =
      config.as[FiniteDuration](s"$configPath.max-timestamp-diff")
    ...
    val blacklistedAddresses =
      config.as[List[String]](s"$configPath.blacklisted-addresses")

    val isMigrateToNewOrderHistoryStorage = !new File(dataDirectory).exists()
    val balanceWatching = config.as[BalanceWatcherWorkerActor.Settings](
      s"$configPath.balance-watching")
