...
class MatcherSnapshotStore(config: Config) extends SnapshotStore {
  ...
  private def migrate(): Unit =
    for (snapshotDir <- Option(new File(config.getString("dir")))
         if snapshotDir.isDirectory) {
      log.info(s"Migrating snapshots from $snapshotDir")

      def snapshotFileForWrite(metadata: SnapshotMetadata,
                               extension: String = ""): File =
        new File(snapshotDir, s"snapshot-${URLEncoder.encode(
          metadata.persistenceId,
          UTF_8.name())}-${metadata.sequenceNr}-${metadata.timestamp}$extension")

      val allSnapshots =
        Option(snapshotDir.listFiles()).fold(Seq.empty[SnapshotMetadata]) {
          fs =>
            fs.map(f => extractMetadata(f.getName)).collect {
              case Some((pid, snr, tms)) =>
                SnapshotMetadata(URLDecoder.decode(pid, UTF_8.name()), snr, tms)
            }
        }
      ...
      if (allSnapshots.nonEmpty) {
        ...
        for (sm <- allSnapshots) {
          ...
          val snapshotInputStream = new BufferedInputStream(
...
object MatcherSnapshotStore extends ScorexLogging {
  ...
  private def extractMetadata(
      filename: String): Option[(String, Long, Long)] = {
    val sequenceNumberEndIdx = filename.lastIndexOf('-')
    val persistenceIdEndIdx =
      filename.lastIndexOf('-', sequenceNumberEndIdx - 1)
    val timestampString = filename.substring(sequenceNumberEndIdx + 1)
    if (persistenceIdStartIdx >= persistenceIdEndIdx || timestampString.exists(
          !_.isDigit)) None
    else {
      val persistenceId =
        filename.substring(persistenceIdStartIdx, persistenceIdEndIdx)
      val sequenceNumber =
        filename.substring(persistenceIdEndIdx + 1, sequenceNumberEndIdx).toLong
