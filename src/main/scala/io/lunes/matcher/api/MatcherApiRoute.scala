...
import io.lunes.matcher.market.MatcherActor.{GetMarkets, GetMarketsResponse}
import io.lunes.matcher.market.MatcherTransactionWriter.GetTransactionsByOrder
...
case class MatcherApiRoute(wallet: Wallet,
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "amountAsset",
                           value = "Amount Asset Id in Pair, or 'LUNES'",
                           dataType = "string",
                           paramType = "path"),
      new ApiImplicitParam(name = "priceAsset",
                           value = "Price Asset Id in Pair, or 'LUNES'",
                           dataType = "string",
                           paramType = "path"),
  ...
  def orderBook: Route = (path("orderbook" / Segment / Segment) & get) {
    (a1, a2) =>
            ...
            case Success(resp) =>
              resp.code match {
                case StatusCodes.Found =>
                  redirect(Uri(s"/matcher/orderbook/$a2/$a1"),
                           StatusCodes.Found)
  ...
  @ApiImplicitParams(
    Array(
      ...
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "io.lunes.matcher.api.CancelOrderRequest"
      )
    ))
  def cancel: Route =
    (path("orderbook" / Segment / Segment / "cancel") & post) { (a1, a2) =>
      withAssetPair(a1, a2) { pair =>
  ...
  @ApiOperation(
    ...
    notes =
      "Delete Order from History by Id if it's in terminal status (Filled, Cancel)",
    httpMethod = "POST",
    produces = "application/json",
    consumes = "application/json"
  )
  ...
  def historyDelete: Route =
    (path("orderbook" / Segment / Segment / "delete") & post) { (a1, a2) =>
      withAssetPair(a1, a2) { pair =>
        json[CancelOrderRequest] { req =>
          if (req.isSignatureValid) {
            (orderHistory ? DeleteOrderFromHistory(pair,
                                                   req.senderPublicKey.address,
                                                   Base58.encode(req.orderId),
                                                   NTP.correctedTime()))
              .mapTo[MatcherResponse]
              .map(r => r.code -> r.json)
          } else {
  ...
  def withAssetPair(a1: String, a2: String): Directive1[AssetPair] = {
    AssetPair.createAssetPair(a1, a2) match {
      ...
      case Failure(_) =>
        complete(
          StatusCodes.BadRequest -> Json.obj("message" -> "Invalid asset pair"))
    }
  }
  ...
  def getAssetPairAndPublicKeyOrderHistory: Route =
    (path("orderbook" / Segment / Segment / "publicKey" / Segment) & get) {
      (a1, a2, publicKey) =>
        (headerValueByName("Timestamp") & headerValueByName("Signature")) {
          (ts, sig) =>
            checkGetSignature(publicKey, ts, sig) match {
              case Success(address) =>
                withAssetPair(a1, a2) { pair =>
                  complete(
                    (orderHistory ? GetOrderHistory(pair,
                                                    address,
                                                    NTP.correctedTime()))
                      .mapTo[MatcherResponse]
                      .map(r => r.code -> r.json))
  ...
  def getPublicKeyOrderHistory: Route = (path("orderbook" / Segment) & get) {
    publicKey =>
  ...
  def checkGetSignature(publicKey: String,
                        timestamp: String,
                        signature: String): Try[String] = {
    Try {
      val pk = Base58.decode(publicKey).get
      val sig = Base58.decode(signature).get
      val ts = timestamp.toLong
      require(math
                .abs(ts - NTP.correctedTime())
                .millis < matcherSettings.maxTimestampDiff,
              "Incorrect timestamp")
      require(crypto.verify(sig, pk ++ Longs.toByteArray(ts), pk),
              "Incorrect signature")
  ...
  def forceCancelOrder: Route =
    (path("orders" / "cancel" / Segment) & post & withAuth) { orderId =>
      implicit val timeout = Timeout(10.seconds)
      val resp: Future[MatcherResponse] =
        (orderHistory ? ForceCancelOrderFromHistory(orderId)).flatMap {
          case Some(order: Order) =>
            (matcher ? ForceCancelOrder(order.assetPair, orderId))
              .mapTo[MatcherResponse]
          case None =>
            Future {
  ...
  def getAllOrderHistory: Route = (path("orders" / Segment) & get & withAuth) {
    addr =>
      implicit val timeout: Timeout = Timeout(10.seconds)
      complete(
        (orderHistory ? GetAllOrderHistory(addr,
                                           activeOnly = true,
                                           NTP.correctedTime()))
          .mapTo[MatcherResponse]
          .map(r => r.code -> r.json))
  ...
  def getTradableBalance: Route =
    (path("orderbook" / Segment / Segment / "tradableBalance" / Segment) & get) {
      (a1, a2, address) =>
        withAssetPair(a1, a2) { pair =>
          complete(
            (orderHistory ? GetTradableBalance(pair,
                                               address,
                                               NTP.correctedTime()))
              .mapTo[MatcherResponse]
              .map(r => r.code -> r.json))
  ...
  def reservedBalance: Route = (path("balance" / "reserved" / Segment) & get) {
    publicKey =>
      (headerValueByName("Timestamp") & headerValueByName("Signature")) {
        (ts, sig) =>
          checkGetSignature(publicKey, ts, sig) match {
            ...
            case Failure(ex) =>
              complete(
                StatusCodes.BadRequest -> Json.obj("message" -> ex.getMessage))
          }
      }
  }
  ...
  def orderStatus: Route =
    (path("orderbook" / Segment / Segment / Segment) & get) {
      (a1, a2, orderId) =>
        withAssetPair(a1, a2) { pair =>
          complete(
  ...
  def orderBookDelete: Route =
    (path("orderbook" / Segment / Segment) & delete & withAuth) { (a1, a2) =>
      withAssetPair(a1, a2) { pair =>
        complete(
  ...
  def getTransactionsByOrder: Route = (path("transactions" / Segment) & get) {
    orderId =>
      complete(
