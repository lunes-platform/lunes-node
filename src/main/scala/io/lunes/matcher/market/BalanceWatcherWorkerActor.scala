...
import io.lunes.matcher.market.BalanceWatcherWorkerActor._
import io.lunes.matcher.market.OrderBookActor.ForceCancelOrder
import io.lunes.matcher.market.OrderHistoryActor.{
  ForceCancelOrderFromHistory,
  GetActiveOrdersByAddress,
  GetActiveOrdersByAddressResponse
}
...
class BalanceWatcherWorkerActor(settings: Settings,
                                matcher: ActorRef,
  ...
  private def waitOrders(taskId: Long,
    ...
    case GetActiveOrdersByAddressResponse(`taskId`, address, orders) =>
      ...
      if (updated.isEmpty) {
        ...
        context.become(
          waitOrders(taskId, stashed, updated, reschedule(waitOrdersTimeout)))
  ...
  private def becomeWorking(event: BalanceChanged): Unit = {
    event.changes.foreach {
      case (addr, changes) =>
        orderHistory ! GetActiveOrdersByAddress(requestIdCounter,
                                                addr,
    ...
    context.become(
      waitOrders(requestIdCounter,
                 BalanceChanged.empty,
                 event,
                 reschedule(EmptyCancellable)))
    requestIdCounter += 1
  }
  ...
  private def reschedule(old: Cancellable): Cancellable = {
    ...
    context.system.scheduler
      .scheduleOnce(settings.oneAddressProcessingTimeout, self, ReceiveTimeout)
  }

  private def replaceChanges(orig: BalanceChanged,
                             replacement: BalanceChanged) =
    BalanceChanged(orig.changes ++ replacement.changes)

  private def ordersToDelete(unprocessed: BalanceChanged,
                             ownerAddress: Address,
                             orders: Seq[LimitOrder]): OrdersToDelete = {
    unprocessed.changes
      .get(ownerAddress)
      .map { changes =>
        val portfolio = changes.updatedPortfolio

        val ordersByPriority =
          orders.sortBy(_.order.timestamp)(Ordering[Long].reverse)
        val (_, r) =
          ordersByPriority.foldLeft((portfolio, List.empty: OrdersToDelete)) {
            case ((restPortfolio, toDelete), limitOrder) =>
              val updatedPortfolio = restPortfolio
                .copy(balance = restPortfolio.balance - restPortfolio.lease.out,
                      lease = LeaseBalance.empty)
                .remove(limitOrder.spentAcc.assetId, limitOrder.getSpendAmount)
                .flatMap { p =>
                  if (limitOrder.rcvAsset == AssetPair.LunesName && limitOrder.getReceiveAmount >= limitOrder.remainingFee)
                    Some(p)
              ...
              updatedPortfolio match {
                ...
                case None =>
                  (restPortfolio,
                   (limitOrder.order.assetPair -> limitOrder.order
                     .idStr()) :: toDelete)
              }
          }
        r
...
object BalanceWatcherWorkerActor {
  ...
  case class Settings(enable: Boolean,
                      oneAddressProcessingTimeout: FiniteDuration)

  def props(settings: Settings,
            matcher: ActorRef,
            orderHistory: ActorRef): Props =
