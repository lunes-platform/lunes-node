...
import io.lunes.matcher.api.{MatcherResponse, StatusCodeMatcherResponse}
import io.lunes.matcher.market.OrderBookActor._
import io.lunes.matcher.model.Events.BalanceChanged
...
import io.lunes.transaction.assets.exchange.{AssetPair, Order, Validation}
...
class MatcherActor(orderHistory: ActorRef,
  ...
  def createOrderBook(pair: AssetPair): ActorRef = {
    ...
    context.actorOf(OrderBookActor.props(pair,
                                         orderHistory,
                                         blockchain,
                                         settings,
                                         wallet,
  ...
  def checkPairOrdering(aPair: AssetPair): Validation = {
    ...
    val isCorrectOrder =
      if (tradedPairs.contains(aPair)) true
      ...
      else if (settings.priceAssets.contains(aPair.priceAssetStr) && settings.priceAssets
                 .contains(aPair.amountAssetStr)) {
        settings.priceAssets.indexOf(aPair.priceAssetStr) < settings.priceAssets
          .indexOf(aPair.amountAssetStr)
        ...
        compare(aPair.priceAsset.map(_.arr), aPair.amountAsset.map(_.arr)) < 0
  ...
  def checkBlacklistRegex(aPair: AssetPair): Validation = {
    val (amountName, priceName) =
      (getAssetName(aPair.amountAsset), getAssetName(aPair.priceAsset))
  ...
  def checkAssetPair[A <: { def assetPair: AssetPair }](msg: A)(
      f: => Unit): Unit = {
    val v = checkBlacklistId(msg.assetPair) && basicValidation(msg) && checkBlacklistRegex(
      msg.assetPair)
    if (!v) {
  ...
  def forwardToOrderBook: Receive = {
    case GetMarkets =>
      sender() ! GetMarketsResponse(getMatcherPublicKey,
                                    tradedPairs.values.toSeq)
    ...
    case x: CancelOrder =>
      checkAssetPair(x) {
        context
          ...
          .fold {
            sender() ! OrderCancelRejected(
              s"Order '${x.orderId}' is already cancelled or never existed in '${x.assetPair.key}' pair")
          }(forwardReq(x))
      }
  ...
  def initPredefinedPairs(): Unit = {
    settings.predefinedPairs
      .diff(tradedPairs.keys.toSeq)
      .foreach(pair => createOrderBook(pair))
  }
  ...
  private def removeOrderBook(pair: AssetPair): Unit = {
    if (tradedPairs.contains(pair)) {
      ...
      persistAll(tradedPairs.map(v => OrderBookCreated(v._1)).to[immutable.Seq]) {
        _ =>
        }
    }
  }
  ...
  private def createBalanceWatcher(): Unit =
    if (settings.balanceWatching.enable) {
      val balanceWatcherMaster =
        context.actorOf(
          FromConfig.props(
            BalanceWatcherWorkerActor
              .props(settings.balanceWatching, self, orderHistory)),
          "balance-watcher-router")
      context.system.eventStream
        .subscribe(balanceWatcherMaster, classOf[BalanceChanged])
    }
}
...
object MatcherActor {
  ...
  case class GetMarketsResponse(publicKey: Array[Byte],
                                markets: Seq[MarketData])
