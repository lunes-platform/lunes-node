...
import io.lunes.matcher.api.{CancelOrderRequest, MatcherResponse}
import io.lunes.matcher.market.OrderBookActor._
import io.lunes.matcher.market.OrderHistoryActor._
...
import io.lunes.matcher.model.MatcherModel._
...
import io.lunes.transaction.assets.exchange._
...
class OrderBookActor(assetPair: AssetPair,
  ...
  private val snapshotCancellable = context.system.scheduler.schedule(
    settings.snapshotsInterval,
    settings.snapshotsInterval,
    self,
    SaveSnapshot)
  private val cleanupCancellable = context.system.scheduler.schedule(
    settings.orderCleanupInterval,
    settings.orderCleanupInterval,
    self,
    OrderCleanup)
  ...
  private def fullCommands: Receive =
    readOnlyCommands orElse snapshotsCommands orElse executeCommands
  ...
  private def snapshotsCommands: Receive = {
    ...
    case DeleteOrderBookRequest(pair) =>
      orderBook.asks.values
        ...
        .flatten
        .foreach(x =>
          context.system.eventStream.publish(Events.OrderCanceled(x)))
    ...
    case DeleteMessagesFailure(cause: Throwable, toSequenceNr: Long) =>
      log.error(
        s"$persistenceId DeleteMessagesFailure up to $toSequenceNr, reason: $cause")
  }
  ...
  private def waitingValidation: Receive = readOnlyCommands orElse {
    ...
    case cancel: CancelOrder
        if Option(cancelInProgressOrders.getIfPresent(cancel.orderId)).nonEmpty =>
      log.info(
        s"Order($assetPair, ${cancel.orderId}) is already being canceled")
  ...
  private def readOnlyCommands: Receive = {
    case GetOrdersRequest =>
      sender() ! GetOrdersResponse(
        orderBook.asks.values.flatten.toSeq ++ orderBook.bids.values.flatten.toSeq)
      ...
      case None =>
        ...
        cancellable = Some(
          context.system.scheduler
            .scheduleOnce(ValidationTimeout, self, ValidationTimeoutExceeded))
  ...
  private def handleValidateCancelResult(
      res: Either[GenericError, String]): Unit = {
    res match {
      case Left(err) =>
  ...
  private def handleGetOrderBook(pair: AssetPair, depth: Option[Int]): Unit = {
    def aggregateLevel(l: (Price, Level[LimitOrder])) =
      LevelAgg(l._1, l._2.foldLeft(0L)((b, o) => b + o.amount))
  ...
  private def handleValidateOrderResult(
      res: Either[GenericError, Order]): Unit = {
    res match {
      case Left(err) =>
      ...
      case Right(o) =>
        log.debug(
          s"Order accepted: '${o.idStr()}' in '${o.assetPair.key}', trying to match ...")
  ...
  private def processInvalidTransaction(
      event: OrderExecuted,
      err: ValidationError): Option[LimitOrder] = {
    ...
    err match {
      case OrderValidationError(order, _) if order == event.submitted.order =>
        None
      case OrderValidationError(order, _) if order == event.counter.order =>
        cancelCounterOrder()
        ...
        (for {
          ...
          case Right(tx) if tx.isInstanceOf[ExchangeTransaction] =>
            ...
            context.system.eventStream.publish(
              ExchangeTransactionCreated(tx.asInstanceOf[ExchangeTransaction]))
            if (event.submittedRemaining > 0)
              Some(o.partial(event.submittedRemaining))
            else None
          case Left(ex) =>
            log.info("Can't create tx for o1: " + Json.prettyPrint(
  ...
  val isMigrateToNewOrderHistoryStorage =
    settings.isMigrateToNewOrderHistoryStorage
  ...
  override def receiveRecover: Receive = {
    ...
    case SnapshotOffer(_, snapshot: Snapshot) =>
      ...
      log.debug(
        s"Recovering OrderBook from snapshot: $snapshot for $persistenceId")
  }
...
object OrderBookActor {
  ...
  case class GetOrderBookRequest(assetPair: AssetPair, depth: Option[Int])
      extends OrderBookRequest

  case class DeleteOrderBookRequest(assetPair: AssetPair)
      extends OrderBookRequest

  case class CancelOrder(assetPair: AssetPair, req: CancelOrderRequest)
      extends OrderBookRequest {
    lazy val orderId: String = Base58.encode(req.orderId)
    override lazy val toString: String =
      s"CancelOrder($assetPair, ${req.senderPublicKey}, $orderId)"
  }

  case class ForceCancelOrder(assetPair: AssetPair, orderId: String)
      extends OrderBookRequest
  ...
  case class GetOrderBookResponse(pair: AssetPair,
                                  bids: Seq[LevelAgg],
                                  asks: Seq[LevelAgg])
      extends MatcherResponse {
    val json: JsValue =
      Json.toJson(OrderBookResult(NTP.correctedTime(), pair, bids, asks))
    val code = StatusCodes.OK
  }

  object GetOrderBookResponse {
    def empty(pair: AssetPair): GetOrderBookResponse =
      GetOrderBookResponse(pair, Seq(), Seq())
  }
