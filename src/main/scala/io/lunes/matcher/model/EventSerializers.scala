...
import io.lunes.matcher.market.MatcherActor.OrderBookCreated
import io.lunes.matcher.market.OrderBookActor.Snapshot
import io.lunes.matcher.market.{MatcherActor, OrderBookActor}
import io.lunes.matcher.model.Events._
import io.lunes.matcher.model.MatcherModel.{Level, Price}
...
class MatcherSerializer extends SerializerWithStringManifest {
    ...
    Json
      .stringify(o match {
        ...
        case obc: MatcherActor.OrderBookCreated =>
          orderBookCreatedFormat.writes(obc)
  ...
  override def fromBinary(bytes: Array[Byte], manifest: String): AnyRef =
    manifest match {
      ...
      case Manifest.OrderBookCreated =>
        orderBookCreatedFormat.reads(Json.parse(bytes)).get
      case Manifest.OrderAdded => orderAddedFormat.reads(Json.parse(bytes)).get
      case Manifest.OrderExecuted =>
        orderExecutedFormat.reads(Json.parse(bytes)).get
      case Manifest.OrderCancelled =>
        orderCancelledFormat.reads(Json.parse(bytes)).get
...
object MatcherSerializer {
  ...
  implicit val limitFormat: Format[LimitOrder] =
    fmt[LimitOrder](LimitOrder.limitOrder)
  implicit val buyFormat: Format[BuyLimitOrder] =
    fmt[BuyLimitOrder](BuyLimitOrder.apply)
  implicit val sellFormat: Format[SellLimitOrder] =
    fmt[SellLimitOrder](SellLimitOrder.apply)
  ...
  implicit val buyOrderTreeMapReads =
    new Reads[TreeMap[Price, Level[BuyLimitOrder]]] {
      override def reads(
          jv: JsValue): JsResult[TreeMap[Price, Level[BuyLimitOrder]]] = {
        val a = jv.as[Map[String, Level[BuyLimitOrder]]].map {
          case (k, v) => (k.toLong, v)
        }
        JsSuccess(
          TreeMap
            .empty[Price, Level[BuyLimitOrder]](OrderBook.bidsOrdering) ++ a)
      }
    }

  implicit val sellOrderTreeMapReads =
    new Reads[TreeMap[Price, Level[SellLimitOrder]]] {
      override def reads(
          jv: JsValue): JsResult[TreeMap[Price, Level[SellLimitOrder]]] = {
        val a = jv.as[Map[String, Level[SellLimitOrder]]].map {
          case (k, v) => (k.toLong, v)
        }
        JsSuccess(
          TreeMap
            .empty[Price, Level[SellLimitOrder]](OrderBook.asksOrdering) ++ a)
      }
    }
  ...
  val orderExecutedFormat = ((__ \ "o1").format[LimitOrder] and
    (__ \ "o2")
      .format[LimitOrder])(OrderExecuted.apply, unlift(OrderExecuted.unapply))
  ...
  implicit val snapshotFormat: Format[Snapshot] =
    Format((JsPath \ "o").read[OrderBook].map(Snapshot),
