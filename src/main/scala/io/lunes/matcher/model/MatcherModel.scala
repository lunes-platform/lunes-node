...
import io.lunes.matcher.model.MatcherModel.Price
...
sealed trait LimitOrder {
  ...
  def feeAmount: Long =
    Try(
      (BigInt(amount) * order.matcherFee / order.amount).bigInteger
        .longValueExact()).getOrElse(Long.MaxValue)
  def remainingAmount: Long = order.amount - amount
  val remainingFee: Long = order.matcherFee - Try(
    (BigInt(remainingAmount) * order.matcherFee / order.amount).bigInteger
      .longValueExact())
    .getOrElse(0L)

  def spentAcc: AssetAcc =
    AssetAcc(order.senderPublicKey, order.getSpendAssetId)
  def rcvAcc: AssetAcc =
    AssetAcc(order.senderPublicKey, order.getReceiveAssetId)
  def feeAcc: AssetAcc = AssetAcc(order.senderPublicKey, None)

  def spentAsset: String =
    order.getSpendAssetId.map(_.base58).getOrElse(AssetPair.LunesName)
  def rcvAsset: String =
    order.getReceiveAssetId.map(_.base58).getOrElse(AssetPair.LunesName)
  def feeAsset: String = AssetPair.LunesName
}

case class BuyLimitOrder(price: Price, amount: Long, order: Order)
    extends LimitOrder {
  def partial(amount: Price): LimitOrder = copy(amount = amount)
  def getReceiveAmount: Long = amount
  def getSpendAmount: Long =
    Try(
      (BigInt(amount) * price / Order.PriceConstant).bigInteger
        .longValueExact()).getOrElse(Long.MaxValue)
}
case class SellLimitOrder(price: Price, amount: Long, order: Order)
    extends LimitOrder {
  def partial(amount: Price): LimitOrder = copy(amount = amount)
...
object LimitOrder {
  ...
  def limitOrder(price: Long, amount: Long, o: Order): LimitOrder =
    o.orderType match {
      case OrderType.BUY  => BuyLimitOrder(price, amount, o).copy()
      case OrderType.SELL => SellLimitOrder(price, amount, o)
    }

  def validateAmount(lo: LimitOrder): Validation =
    lo.order.isValidAmount(lo.price, lo.amount)
}

object Events {
  sealed trait Event
  case class OrderExecuted(submitted: LimitOrder, counter: LimitOrder)
      extends Event {
    ...
    def submittedRemaining: Long =
      math.max(submitted.amount - counter.amount, 0)
    def submittedRemainingOrder: LimitOrder =
      submitted.partial(submittedRemaining)
  ...
  object BalanceChanged {
    ...
    case class Changes(updatedPortfolio: Portfolio,
                       changedAssets: Set[Option[AssetId]])
  }
  ...
  def createOrderInfo(event: Event): Map[String, (Order, OrderInfo)] = {
    event match {
      case OrderAdded(lo) =>
        Map(
          (lo.order.idStr(), (lo.order, OrderInfo(lo.order.amount, 0L, false))))
      case oe: OrderExecuted =>
        val (o1, o2) = (oe.submittedExecuted, oe.counterExecuted)
        Map(
          (o1.order.idStr(),
           (o1.order, OrderInfo(o1.order.amount, o1.amount, false))),
          (o2.order.idStr(),
           (o2.order, OrderInfo(o2.order.amount, o2.amount, false)))
        )
  ...
  def createOpenPortfolio(event: Event): Map[String, OpenPortfolio] = {
    def overdraftFee(lo: LimitOrder): Long = {
      if (lo.feeAcc == lo.rcvAcc)
        math.max(lo.feeAmount - lo.getReceiveAmount, 0L)
      else lo.feeAmount
    }
    ...
    event match {
      ...
      case OrderCanceled(lo) =>
        val feeDiff =
          if (lo.feeAcc == lo.rcvAcc)
            math.max(lo.remainingFee - lo.getReceiveAmount, 0L)
          else lo.remainingFee
        Map(
