...
object OrderBook {
  ...
  val empty: OrderBook =
    OrderBook(TreeMap.empty[Price, Level[BuyLimitOrder]](bidsOrdering),
  ...
  def cancelOrder(ob: OrderBook, orderId: String): Option[OrderCanceled] = {
    ob.bids
      ...
      .orElse(ob.asks.find {
        case (_, v) => v.exists(_.order.idStr() == orderId)
      })
  ...
  private def updateCancelOrder(ob: OrderBook,
                                limitOrder: LimitOrder): (OrderBook) = {
    (limitOrder match {
  ...
  private def updateExecutedBuy(ob: OrderBook, o: BuyLimitOrder, r: Long) =
    ob.bids.get(o.price) match {
      case Some(l) =>
        val (l1, l2) = l.span(_ != o)
        val ll =
          if (r > 0) (l1 :+ o.copy(amount = r)) ++ l2.tail else l1 ++ l2.tail
        ob.copy(
          bids =
            if (ll.isEmpty) ob.bids - o.price else ob.bids + (o.price -> ll))
      case None => ob
    }

  private def updateExecutedSell(ob: OrderBook, o: SellLimitOrder, r: Long) =
    ob.asks.get(o.price) match {
      case Some(l) =>
        val (l1, l2) = l.span(_ != o)
        val ll =
          if (r > 0) (l1 :+ o.copy(amount = r)) ++ l2.tail else l1 ++ l2.tail
        ob.copy(
          asks =
            if (ll.isEmpty) ob.asks - o.price else ob.asks + (o.price -> ll))
      case None => ob
    }
  ...
  def updateState(ob: OrderBook, event: Event): OrderBook = event match {
    ...
    case e @ OrderExecuted(_, c: BuyLimitOrder) =>
      updateExecutedBuy(ob, c, e.counterRemaining)
    case e @ OrderExecuted(_, c: SellLimitOrder) =>
      updateExecutedSell(ob, c, e.counterRemaining)
