...
import io.lunes.matcher.model.LimitOrder.{Filled, OrderStatus}
import io.lunes.matcher.model.OrderHistory.OrderHistoryOrdering
import io.lunes.state._
import org.iq80.leveldb.DB
import play.api.libs.json.Json
import io.lunes.transaction.{AssetAcc, AssetId}
import io.lunes.transaction.assets.exchange.{AssetPair, Order, OrderType}
...
trait OrderHistory {
  ...
  def fetchOrderHistoryByPair(
      assetPair: AssetPair,
      address: String): Seq[(String, OrderInfo, Option[Order])]

  def fetchAllActiveOrderHistory(
      address: String): Seq[(String, OrderInfo, Option[Order])]

  def fetchAllOrderHistory(
      address: String): Seq[(String, OrderInfo, Option[Order])]
...
object OrderHistory {
  ...
  object OrderHistoryOrdering
      extends Ordering[(String, OrderInfo, Option[Order])] {
    def orderBy(oh: (String, OrderInfo, Option[Order])): (OrderStatus, Long) =
      (oh._2.status, -oh._3.map(_.timestamp).getOrElse(0L))

    override def compare(first: (String, OrderInfo, Option[Order]),
                         second: (String, OrderInfo, Option[Order])): Int = {
      implicitly[Ordering[(OrderStatus, Long)]]
...
case class OrderHistoryImpl(db: DB, settings: MatcherSettings)
  ...
  def addToFinal(address: String, orderId: String): Unit = {
    get(OrderIdsCodec)(makeKey(AddressToOrdersPrefix, address)) match {
      case Some(prev) =>
        ...
        put(makeKey(AddressToOrdersPrefix, address),
            OrderIdsCodec.encode(r :+ orderId),
            None)
      case _ =>
        put(makeKey(AddressToOrdersPrefix, address),
            OrderIdsCodec.encode(Array(orderId)),
            None)
    }
  }
  ...
  private def saveOrderInfo(event: Event): Map[String, (Order, OrderInfo)] = {
    ...
    updatedInfo.foreach {
      case (orderId, (_, oi)) =>
        put(makeKey(OrdersInfoPrefix, orderId),
            oi.jsonStr.getBytes(Charset),
            None)
  ...
  def openPortfolio(address: String): OpenPortfolio = {
    get(PortfolioCodec)(makeKey(AddressPortfolioPrefix, address))
      .map(OpenPortfolio.apply)
      .getOrElse(OpenPortfolio.empty)
  }
    ...
    updatedInfo.foreach {
      case (orderId, (o, oi)) =>
        if (!oi.status.isFinal) {
          val assetId =
            if (o.orderType == OrderType.BUY) o.assetPair.priceAsset
            else o.assetPair.amountAsset
  ...
  override def orderInfo(id: String): OrderInfo =
    get(makeKey(OrdersInfoPrefix, id))
      .map(Json.parse)
      .flatMap(_.validate[OrderInfo].asOpt)
      .getOrElse(OrderInfo.empty)

  override def order(id: String): Option[Order] = {
    import io.lunes.transaction.assets.exchange.OrderJson.orderFormat
    get(makeKey(OrdersPrefix, id))
      .map(b => new String(b, Charset))
      .map(Json.parse)
      .flatMap(_.validate[Order].asOpt)
  }
  ...
  override def openVolume(assetAcc: AssetAcc): Long = {
    val asset = assetAcc.assetId.map(_.base58).getOrElse(AssetPair.LunesName)
  ...
  override def allOrderIdsByAddress(address: String): Set[String] = {
    get(AssetIdOrderIdSetCodec)(makeKey(AddressToActiveOrdersPrefix, address))
      .getOrElse(Set.empty)
      .map(_._2) ++
      get(OrderIdsCodec)(makeKey(AddressToOrdersPrefix, address))
        .map(_.toSet)
        .getOrElse(Set.empty)
  }
  ...
  override def activeOrderIdsByAddress(
      address: String): Set[(Option[AssetId], String)] = {
    get(AssetIdOrderIdSetCodec)(makeKey(AddressToActiveOrdersPrefix, address))
      .getOrElse(Set.empty)
  }
  ...
  override def fetchOrderHistoryByPair(
      assetPair: AssetPair,
      address: String): Seq[(String, OrderInfo, Option[Order])] = {
  ...
  override def fetchAllActiveOrderHistory(
      address: String): Seq[(String, OrderInfo, Option[Order])] = {
    import OrderInfo.orderStatusOrdering
  ...
  override def fetchAllOrderHistory(
      address: String): Seq[(String, OrderInfo, Option[Order])] = {
    import OrderInfo.orderStatusOrdering
    allOrderIdsByAddress(address).toSeq
  ...
  private def deleteFromOrdersInfo(orderId: String): Unit =
    delete(makeKey(OrdersInfoPrefix, orderId), None)
  ...
  private def addToActive(address: String,
  ...
  private def deleteFromActive(address: String, orderId: String): Unit = {
    update(AssetIdOrderIdSetCodec)(
      makeKey(AddressToActiveOrdersPrefix, address)) { orig =>
      orig.filterNot { case (_, currOrderId) => currOrderId == orderId }
    }
  }

  private def deleteOrderFromAddress(keyPrefix: Array[Byte],
                                     address: String,
                                     orderId: String): Unit = {
  ...
  private def update[Content](codec: Codec[Content])(key: Array[Byte])(
      f: Content => Content): Unit = {
    get(codec)(key).foreach { orig =>
      put(key, codec.encode(f(orig)), None)
    }
  }

  private def get[Content](codec: Codec[Content])(
      key: Array[Byte]): Option[Content] = {
    get(key).map { x =>
