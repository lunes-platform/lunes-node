...
object OrderInfo {
  def safeSum(x: Long, y: Long): Long =
    Try(Math.addExact(x, y)).getOrElse(Long.MaxValue)
  ...
  implicit val orderStatusOrdering: Ordering[LimitOrder.OrderStatus] =
    (first: LimitOrder.OrderStatus, second: LimitOrder.OrderStatus) => {
...
object OpenPortfolio {
  ...
  implicit val orderPortfolioMonoid = new Monoid[OpenPortfolio] {
    ...
    override def combine(lhs: OpenPortfolio,
                         rhs: OpenPortfolio): OpenPortfolio = {
