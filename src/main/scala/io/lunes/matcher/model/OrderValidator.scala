...
import io.lunes.matcher.market.OrderBookActor.CancelOrder
import io.lunes.matcher.model.Events.OrderAdded
...
trait OrderValidator {
  ...
  lazy val matcherPubKey: PublicKeyAccount =
    wallet.findPrivateKey(settings.account).explicitGet()
  ...
  def isBalanceWithOpenOrdersEnough(order: Order): Validation = {
    ...
    val b: Map[String, Long] = (Map(lo.spentAcc -> 0L) ++ Map(lo.feeAcc -> 0L))
      ...
      .map {
        case (a, v) =>
          a.assetId.map(_.base58).getOrElse(AssetPair.LunesName) -> v
      }

    val newOrder = Events
      .createOpenPortfolio(OrderAdded(lo))
      .getOrElse(order.senderPublicKey.address, OpenPortfolio.empty)
    val open =
      orderHistory.openPortfolio(order.senderPublicKey.address).orders.filter {
  ...
  def validateNewOrder(order: Order): Either[GenericError, Order] = {
    val v =
      (order.matcherPublicKey == matcherPubKey) :| "Incorrect matcher public key" &&
        (order.expiration > NTP
          .correctedTime() + MinExpiration) :| "Order expiration should be > 1 min" &&
  ...
  def validateCancelOrder(
      cancel: CancelOrder): Either[GenericError, CancelOrder] = {
