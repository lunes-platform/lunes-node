...
import io.lunes.consensus.{GeneratingBalanceProvider, PoSSelector}
...
import io.lunes.state.appender.{BlockAppender, MicroblockAppender}
...
class MinerImpl(allChannels: ChannelGroup,
  ...
  private val blockBuildTimeStats = Kamon.metrics
    .histogram("pack-and-forge-block-time", instrument.Time.Milliseconds)
  private val microBlockBuildTimeStats = Kamon.metrics
    .histogram("forge-microblock-time", instrument.Time.Milliseconds)
  ...
  def collectNextBlockGenerationTimes: List[(Address, Long)] =
    Await.result(Task.now(nextBlockGenerationTimes.toList).runAsyncLogErr,
                 Duration.Inf)
  ...
  private def forgeBlock(account: PrivateKeyAccount, balance: Long)
    ...
    measureSuccessful(
      ...
      for {
        ...
        mdConstraint = MultiDimensionalMiningConstraint(estimators.total,
                                                        estimators.keyBlock)
        (unconfirmed, updatedMdConstraint) = utx
          .packUnconfirmed(mdConstraint, isSortingRequired())
        _ = log.debug(
  ...
  private def isSortingRequired(): Boolean =
    blockchainUpdater.height <= blockchainSettings.functionalitySettings.dontRequireSortedTransactionsAfter
  ...
  private def generateOneMicroBlockTask(
    ...
    if (pc < minerSettings.quorum) {
      log.trace(
        s"Quorum not available ($pc/${minerSettings.quorum}, not forging microblock with ${account.address}")
      Task.now(Retry)
    } else if (utx.size == 0) {
      ...
      val (unconfirmed, updatedTotalConstraint) =
        measureLog("packing unconfirmed transactions for microblock") {
          ...
          val (unconfirmed, updatedMdConstraint) =
            utx.packUnconfirmed(mdConstraint, sortInBlock = false)
      ...
      if (unconfirmed.isEmpty) {
        ...
        (for {
          ...
          _ <- EitherT(
            MicroblockAppender(checkpoint,
                               blockchainUpdater,
                               utx,
                               appenderScheduler)(microBlock))
  ...
  private def generateBlockTask(account: PrivateKeyAccount): Task[Unit] = {
    {
      ...
      for {
        ...
        balanceAndTs <- nextBlockGenerationTime(
      ...
      case Right((offset, balance)) =>
        ...
        nextBlockGenerationTimes += account.toAddress -> (System
          .currentTimeMillis() + offset.toMillis)
        generateOneBlockTask(account, balance)(offset).flatMap {
          case Right((estimators, block, totalConstraint)) =>
            BlockAppender(checkpoint,
                          blockchainUpdater,
                          timeService,
                          utx,
                          pos,
...
object Miner {
  ...
  val Disabled = new Miner with MinerDebugInfo {
    ...
    override def collectNextBlockGenerationTimes: List[(Address, Long)] =
      List.empty
