...
object MiningConstraints {
  ...
  def apply(minerSettings: MinerSettings,
            blockchain: Blockchain,
            height: Int): MiningConstraints = {
    ...
    new MiningConstraints(
      ...
      keyBlock =
        if (isMassTransferEnabled)
          ...
          val maxTxsForKeyBlock =
            if (isNgEnabled) minerSettings.maxTransactionsInKeyBlock
            else ClassicAmountOfTxsInBlock
      ...
      micro =
        if (isNgEnabled)
          OneDimensionalMiningConstraint(
            minerSettings.maxTransactionsInMicroBlock,
            TxEstimators.one)
