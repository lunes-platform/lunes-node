...
import io.lunes.network.MessageObserver.Messages
...
object NetworkServer extends ScorexLogging {
  ...
  def apply(settings: LunesSettings,
    ...
    val handshake = Handshake(Constants.ApplicationName + settings.blockchainSettings.addressSchemeCharacter, Constants.VersionTuple,
