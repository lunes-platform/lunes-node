...
import io.lunes.mining.{MiningConstraint, MiningConstraints, MultiDimensionalMiningConstraint}
...
class BlockchainUpdaterImpl(blockchain: Blockchain, settings: LunesSettings, time: Time)
  ...
  override def lunesDistribution(height: Int): Map[Address, Long] = ngState.fold(blockchain.lunesDistribution(height)) { ng =>
