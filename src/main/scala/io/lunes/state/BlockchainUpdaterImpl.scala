...
import io.lunes.state.reader.{CompositeBlockchain, LeaseDetails}
import io.lunes.utils.{UnsupportedFeature, forceStopApplication}
import kamon.Kamon
...
import io.lunes.transaction.lease._
...
class BlockchainUpdaterImpl(blockchain: Blockchain,
  ...
  import io.lunes.state.BlockchainUpdaterImpl._
  ...
  private var restTotalConstraint: MiningConstraint = MiningConstraints(
    settings.minerSettings,
    blockchain,
    blockchain.height).total
            ...
            blockchain.lastBlockId match {
              ...
              case lastBlockId =>
                ...
                val miningConstraints =
                  MiningConstraints(settings.minerSettings, blockchain, height)
                BlockDiffer
            ...
            if (ng.base.reference == block.reference) {
              if (block.blockScore() > ng.base.blockScore()) {
                ...
                if (block.transactionData
                  ...
                  val height = blockchain.unsafeHeightOf(ng.base.reference)
                  val miningConstraints =
                    MiningConstraints(settings.minerSettings,
                                      blockchain,
                                      height)
                ...
                case None =>
                  Left(
                    BlockAppendError(
                      ...
                      block))
                case Some(
                    (referencedForgedBlock, referencedLiquidDiff, discarded)) =>
                  if (referencedForgedBlock.signaturesValid().isRight) {
                    ...
                    val expiredTransactions = blockchain.forgetTransactions(
                      (_, txTs) => block.timestamp - txTs > 2 * 60 * 60 * 1000)
                    ...
                    diff.left.foreach { _ =>
                      log.trace(
                        s"Could not append new block, remembering ${expiredTransactions.size} transaction(s)")
  ...
  override def removeAfter(
      blockId: ByteStr): Either[ValidationError, Seq[Block]] = {
    val ng = ngState
  ...
  override def processMicroBlock(
      microBlock: MicroBlock): Either[ValidationError, Unit] = {
    ngState match {
      case None =>
        ...
        ng.lastMicroBlock match {
          ...
          case _ =>
            for {
              ...
              r <- {
                val constraints = MiningConstraints(settings.minerSettings,
                                                    blockchain,
                                                    blockchain.height)
  ...
  override def blockHeaderAndSize(
      blockId: BlockId): Option[(BlockHeader, Int)] =
    liquidBlockHeaderAndSize().filter(_._1.uniqueId == blockId) orElse blockchain
  ...
  override def addressTransactions(address: Address,
    ...
    ngState.fold(blockchain.addressTransactions(address, types, count, from)) {
      ng =>
        ...
        if (actualTxCount == count) transactionsFromDiff
          ...
          transactionsFromDiff ++ blockchain.addressTransactions(
            address,
            types,
            count - actualTxCount,
            0)
  ...
  override def containsTransaction(id: AssetId): Boolean =
    ngState.fold(blockchain.containsTransaction(id)) { ng =>
      ng.bestLiquidDiff.transactions.contains(id) || blockchain
        .containsTransaction(id)
    }

  override def forgetTransactions(
      pred: (AssetId, Long) => Boolean): Map[AssetId, Long] =
    blockchain.forgetTransactions(pred)

  override def learnTransactions(values: Map[AssetId, Long]): Unit =
    blockchain.learnTransactions(values)
  ...
  override def hasScript(address: Address): Boolean =
    ngState.fold(blockchain.hasScript(address)) { ng =>
      ng.bestLiquidDiff.scripts.contains(address) || blockchain.hasScript(
        address)
    }
  ...
  override def assetDistribution(assetId: AssetId): Map[Address, Long] =
    blockchain.assetDistribution(assetId) ++ changedBalances(
      _.assets.getOrElse(assetId, 0L) != 0,
      portfolio(_).assets.getOrElse(assetId, 0L))

  override def lunesDistribution(height: Int): Map[Address, Long] =
    ngState.fold(blockchain.lunesDistribution(height)) { ng =>
      val innerDistribution = blockchain.lunesDistribution(height)
      if (height < this.height) innerDistribution
      else {
        innerDistribution ++ changedBalances(_.balance != 0,
                                             portfolio(_).balance)
      }
    }
...
object BlockchainUpdaterImpl extends ScorexLogging {
  ...
  private val microBlockForkHeightStats =
    Kamon.metrics.histogram("micro-block-fork-height")
  private val forgeBlockTimeStats =
    Kamon.metrics.histogram("forge-block-time", KTime.Milliseconds)
