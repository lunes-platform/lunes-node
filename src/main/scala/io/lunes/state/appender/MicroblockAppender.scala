...
object MicroblockAppender extends ScorexLogging with Instrumented {
  ...
  def apply(checkpoint: CheckpointService,
    ...
    Task(
      measureSuccessful(
        ...
        for {
          _ <- Either.cond(
            checkpoint.isBlockValid(microBlock.totalResBlockSig,
                                    blockchainUpdater.height + 1),
            (),
            MicroBlockAppendError(
              s"[h = ${blockchainUpdater.height + 1}] is not valid with respect to checkpoint",
              microBlock)
          )
  ...
  def apply(
    ...
    (for {
      ...
      validApplication <- EitherT(
        apply(checkpoint, blockchainUpdater, utxStorage, scheduler)(microBlock))
  ...
  private val microblockProcessingTimeStats =
    Kamon.metrics.histogram("microblock-processing-time")
}
