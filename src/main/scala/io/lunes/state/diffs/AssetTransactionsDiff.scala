...
object AssetTransactionsDiff {
  def issue(height: Int)(
    ...
    val info = AssetInfo(isReissuable = tx.reissuable,
                         volume = tx.quantity,
                         script = tx.script)
    Right(
      Diff(
        ...
        if ((Long.MaxValue - tx.quantity) < oldInfo.totalVolume && isDataTxActivated) {
          ...
          Right(
            Diff(
              ...
              assetInfos = Map(
                tx.assetId -> AssetInfo(volume = tx.quantity,
      ...
      Diff(
        ...
        portfolios = Map(
          tx.sender.toAddress -> Portfolio(balance = -tx.fee,
                                           lease = LeaseBalance.empty,
                                           assets =
                                             Map(tx.assetId -> -tx.quantity))),
        assetInfos = Map(
          tx.assetId -> AssetInfo(isReissuable = true,
                                  volume = -tx.quantity,
                                  None))
      )
    }
        ...
        Diff(
          ...
          portfolios = Map(
            tx.sender.toAddress -> Portfolio(balance = -tx.fee,
                                             lease = LeaseBalance.empty,
                                             assets = Map.empty)),
          sponsorship = Map(tx.assetId -> SponsorshipValue(
            tx.minSponsoredAssetFee.getOrElse(0)))
        ))
    }
  }
  ...
  private def validateAsset(
    ...
    blockchain.transactionInfo(assetId) match {
      case Some((_, sitx: IssueTransaction))
          if sitx.script.isEmpty && !validIssuer(issuerOnly,
                                                 tx.sender,
                                                 sitx.sender) =>
