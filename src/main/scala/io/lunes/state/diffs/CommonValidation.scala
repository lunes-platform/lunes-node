...
import io.lunes.features.{BlockchainFeature, BlockchainFeatures}
      ...
      if (height < Sponsorship.sponsoredFeesSwitchHeight(blockchain, fs))
        ...
        for {
          ...
          _ <- Either.cond(
            ...
            (),
            GenericError(
              s"Fee in ${feeAssetId.fold("LUNES")(_.toString)} for ${tx.builder.classTag} does not exceed minimal value of $minimumFee LUNES: $feeAmount")
          )
