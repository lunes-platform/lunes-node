...
import io.lunes.transaction.transfer.MassTransferTransaction.ParsedTransfer
  ...
  def apply(blockchain: Blockchain, blockTime: Long, height: Int)(
    ...
    portfoliosEi.flatMap { list: List[(Map[Address, Portfolio], Long)] =>
      ...
      val completePortfolio = recipientPortfolios.combine(tx.assetId match {
        case None =>
          Map(sender -> Portfolio(-totalAmount, LeaseBalance.empty, Map.empty))
        case Some(aid) =>
          Map(
            sender -> Portfolio(0,
                                LeaseBalance.empty,
      ...
      val assetIssued =
        tx.assetId.forall(blockchain.assetDescription(_).isDefined)
