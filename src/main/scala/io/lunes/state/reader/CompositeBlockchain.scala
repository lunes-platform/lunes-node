...
class CompositeBlockchain(inner: Blockchain, maybeDiff: => Option[Diff])
    extends Blockchain {
    ...
    inner.assetDescription(id) match {
      case Some(ad) =>
        diff.issuedAssets
          ...
          .map { newAssetInfo =>
            val oldAssetInfo =
              AssetInfo(ad.reissuable, ad.totalVolume, ad.script)
  ...
  override def addressTransactions(address: Address,
    ...
    if (actualTxCount == count) transactionsFromDiff
      ...
      transactionsFromDiff ++ inner.addressTransactions(address,
                                                        types,
                                                        count - actualTxCount,
                                                        0)
    }
  ...
  override def containsTransaction(id: ByteStr): Boolean =
    diff.transactions.contains(id) || inner.containsTransaction(id)

  override def forgetTransactions(pred: (AssetId, Long) => Boolean) =
    inner.forgetTransactions(pred)
  override def learnTransactions(values: Map[AssetId, Long]): Unit =
    inner.learnTransactions(values)
  ...
  override def assetDistribution(assetId: ByteStr): Map[Address, Long] =
    inner.assetDistribution(assetId) ++ changedBalances(
  ...
  override def lunesDistribution(height: Int): Map[Address, Long] = {
    val innerDistribution = inner.lunesDistribution(height)
...
object CompositeBlockchain {
  ...
  def composite(inner: Blockchain, diff: Diff): Blockchain =
