...
trait SnapshotStateReader extends Synchronized {
   ...
   * @return
   *   Returns a Map for [[scorex.account.Address]] into [[io.lunes.state2.Portfolio]].
   */
   ...
   * @param id
   *   Inputs the Transaction ID.
   * @return
   *   Returns an Option for a Tuple (Int, Option[Transaction]).
   */
   ...
   * @param a
   *   Inputs an [[scorex.account.Address]].
   * @return
   ...
   *   Inputs Asset ID.
   * @return
   *   Returns an Option for [[io.lunes.state2.AssetInfo]].
   */
   ...
   *   Inputs an [[scorex.account.Address]].
   * @return
   *   Returns a Tuple (Long, LeaseInfo).
   */
   ...
   * @param a
   *   Inputs a [[scorex.account.Address]].
   * @param limit
   *   Sets a limit for transaction retrieval.
   * @return
   *   Returns a Seguence of IDs for Transactions.
   */
   ...
   *   Inputs a [[scorex.account.Address]]
   * @return
   *   Returns a Sequence of [[scorex.account.Alias]].
   */
   ...
   * @param a
   *   Inputs a [[scorex.account.Alias]].
   * @return
   *   Returns an Option for [[scorex.account.Address]].
   */
   ...
   * @param leaseTx
   *   Inpust a [[io.lunes.transaction.lease.LeaseTransaction]].
   * @return
   *   Returns true if the Lease is active.
   */
   ...
   * @return
   *   Returns a Sequence of IDs for Active Leases.
   */
   ...
   *   Inputs an [[scorex.account.Address]].
   * @return
   *   Returns an Option for the Height.
   */
   ...
   * @param acc
   *   Inputs the [[scorex.account.Address]].
   * @param h
   *   Inputs the Height.
   * @return
   *   Returns an Option for [[io.lunes.state2.Snapshot]].
   */
   ...
   *   Inpúts the order ID.
   * @return
   *   Return an [[io.lunes.state2.OrderFillInfo]].
   */
...
object SnapshotStateReader {
   ...
   * @constructor
   *   Creates the Extension provideded a SnapshotStateReader Object.
   * @param s
   *   Inputs the [[SnapshotStateReader]].
   */
  implicit class StateReaderExt(s: SnapshotStateReader) extends ScorexLogging {
     ...
     * @param signature
     *   The [[ByteStr]] for the signature.
     * @tparam T
     *   The Parameter Type which must be a Class Descendant of [[io.lunes.transaction.Transaction]].
     * @return
     *   Returs an Option
     */
     ...
     * @param aoa
     *   The Addres or Alias object.
     * @tparam T
     *   The Parametrized Type which must be a descendant of [[io.lunes.transaction.Transaction]].
     * @return
     *   Returns Either a Address (case Success) of a ValidationError (case Failure).
     */
     ...
     * @param signature
     *   The [[ByteStr]] signature.
     * @return
     *   Returns an Option for the Number of elements.
     */
     ...
     * @param account
     *   The [[scorex.account.Address]] of the node.
     * @param limit
     *   The limit of search.
     * @return
     *   Returns a Sequence of Tuples (Int, SomeClass). Such SomeClass must be a descendant of Transaction.
     */
    ...
    def assetDistribution(assetId: Array[Byte]): Map[String, Long] =
      s.assetDistribution(ByteStr(assetId)).map { case (acc, amt) => (acc.address, amt) }

    /**
     * Gets the Effective Balance for a Account.
     * @param account
     *   The [[scorex.account.Address]].
     * @return
     ...
     *   The Asset ID.
     * @return
     *   Returns true if the Asset is Reissuable.
     */
     ...
     * @param acc
     *   The Account [[scorex.account.Address]].
     * @param atHeight
     *   The Starting Height.
     * @param confirmations
     *   Number of Confirmations.
     * @param extractor
     *   An Extrator Map for Snapshot into Long.
     * @return
     *   The Minimum.
     */
            ...
            s.snapshotAtHeight(acc, deeperHeight) match {
              case Some(snapshot) =>
                if (deeperHeight <= bottomNotIncluded)
                  ...
                  throw new Exception(
                    s"CRITICAL: Infinite loop detected while calculating minBySnapshot: acc=$acc, atHeight=$atHeight, " +
                      s"confirmations=$confirmations; lastUpdateHeight=${s
                          .lastUpdateHeight(acc)}; current step: deeperHeight=$deeperHeight, list.size=${list.size}"
                  )
              ...
              case None =>
                throw new Exception(
                  s"CRITICAL: Cannot lookup referenced height: acc=$acc, atHeight=$atHeight, " +
                    s"confirmations=$confirmations; lastUpdateHeight=${s
                        .lastUpdateHeight(acc)}; current step: deeperHeight=$deeperHeight, list.size=${list.size}"
                )
            }
     ...
     *   The Confirmations.
     * @return
     *   Returns a Try object for the effective balance.
     */
    def effectiveBalanceAtHeightWithConfirmations(acc: Address, atHeight: Int, confirmations: Int): Try[Long] =
      Try { // Perhaps Option should be better.
     ...
     *   The Account [[scorex.account.Address]].
     * @param assets
     *   Inputs a Set of [[AssetId]].
     * @return
     *   Returns the partial portifolio.
     */
