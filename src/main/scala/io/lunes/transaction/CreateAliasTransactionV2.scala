...
final case class CreateAliasTransactionV2 private (version: Byte,
...
object CreateAliasTransactionV2
  ...
  override protected def parseTail(
    ...
    Try {
      for {
        (sender, alias, fee, timestamp, end) <- CreateAliasTransaction
          .parseBase(0, bytes)
        result <- (for {
          proofs <- Proofs.fromBytes(bytes.drop(end))
          tx <- CreateAliasTransactionV2.create(version,
                                                sender,
                                                alias,
                                                fee,
                                                timestamp,
  ...
  def create(
    ...
    if (fee <= 0) {
      ...
      Right(
        CreateAliasTransactionV2(version,
                                 sender,
                                 alias,
                                 fee,
                                 timestamp,
