...
object DataTransaction
  ...
  override protected def parseTail(version: Byte,
    ...
    Try {
      ...
      val (entries, p1) =
        if (entryCount > 0) {
          val parsed =
            List.iterate(DataEntry.parse(bytes, p0 + 2), entryCount) {
              case (e, p) => DataEntry.parse(bytes, p)
            }
      ...
      txEi.fold(left => Failure(new Exception(left.toString)),
                right => Success(right))
    }.flatten
  ...
  def create(version: Byte,
    ...
    if (!supportedVersions.contains(version)) {
      ...
      val tx =
        DataTransaction(version, sender, data, feeAmount, timestamp, proofs)
      Either.cond(tx.bytes().length <= MaxBytes,
                  tx,
  ...
  def signed(
    ...
    create(version, sender, data, feeAmount, timestamp, Proofs.empty).right
      .map { unsigned =>
        unsigned.copy(
          proofs = Proofs
