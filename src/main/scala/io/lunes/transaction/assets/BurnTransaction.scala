...
object BurnTransaction {
  ...
  def parseBase(start: Int, bytes: Array[Byte])
    : (PublicKeyAccount, AssetId, Long, Long, Long, Int) = {
    val sender = PublicKeyAccount(bytes.slice(start, start + KeyLength))
    val assetId = ByteStr(
      bytes.slice(start + KeyLength, start + KeyLength + AssetIdLength))
    val quantityStart = start + KeyLength + AssetIdLength

    val quantity =
      Longs.fromByteArray(bytes.slice(quantityStart, quantityStart + 8))
    val fee =
      Longs.fromByteArray(bytes.slice(quantityStart + 8, quantityStart + 16))
    val timestamp =
      Longs.fromByteArray(bytes.slice(quantityStart + 16, quantityStart + 24))
