...
object BurnTransactionV1
  ...
  override protected def parseTail(version: Byte,
    ...
    Try {
      val (sender, assetId, quantity, fee, timestamp, end) =
        BurnTransaction.parseBase(0, bytes)
  ...
  def create(sender: PublicKeyAccount,
    ...
    BurnTransaction
      ...
      .map(_ =>
        BurnTransactionV1(sender, assetId, quantity, fee, timestamp, signature))
  ...
  def signed(sender: PublicKeyAccount,
    ...
    create(sender, assetId, quantity, fee, timestamp, ByteStr.empty).right.map {
      unverified =>
        unverified.copy(
