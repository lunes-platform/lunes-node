...
case class ReissueTransaction private (
  ...
  override val transactionType: TransactionType.Value =
    TransactionType.ReissueTransaction
  ...
  def parseTail(bytes: Array[Byte]): Try[ReissueTransaction] = Try {
    ...
    val assetId = ByteStr(
      bytes.slice(
        ...
        SignatureLength + KeyLength + AssetIdLength + 1
      )
    )
    ...
    val reissuable =
      bytes.slice(quantityStart + 8, quantityStart + 9).head == (1: Byte)
    val fee =
      Longs.fromByteArray(bytes.slice(quantityStart + 9, quantityStart + 17))
    val timestamp =
      Longs.fromByteArray(bytes.slice(quantityStart + 17, quantityStart + 25))
    ReissueTransaction
