...
import io.lunes.transaction.ValidationError.{GenericError, UnsupportedVersion}
...
object LeaseCancelTransactionV2
  ...
  override protected def parseTail(version: Byte,
    ...
    Try {
      ...
      val (sender, fee, timestamp, leaseId, end) =
        LeaseCancelTransaction.parseBase(bytes, 1)
      (for {
        proofs <- Proofs.fromBytes(bytes.drop(end))
        tx <- LeaseCancelTransactionV2.create(version,
                                              chainId,
                                              sender,
                                              leaseId,
                                              fee,
  ...
  def create(version: Byte,
    ...
    for {
      ...
      LeaseCancelTransactionV2(version,
                               chainId,
                               sender,
                               leaseId,
                               fee,
  ...
  def signed(
    ...
    create(version, chainId, sender, leaseId, fee, timestamp, Proofs.empty).right
      .map { unsigned =>
        unsigned.copy(
          proofs = Proofs
