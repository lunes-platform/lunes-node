...
case class SetScriptTransaction private (version: Byte,
  ...
  override val json = Coeval.evalOnce(
    jsonBase() ++ Json.obj("version" -> version,
...
object SetScriptTransaction
  ...
  override protected def parseTail(version: Byte,
    ...
    Try {
      ...
      val timestamp =
        Longs.fromByteArray(bytes.slice(scriptEnd + 8, scriptEnd + 16))
      (for {
        scriptOpt <- scriptEiOpt
        _ <- Either.cond(chainId == networkByte,
                         (),
                         GenericError(s"Wrong chainId ${chainId.toInt}"))
        proofs <- Proofs.fromBytes(bytes.drop(scriptEnd + 16))
        tx <- create(version, sender, scriptOpt, fee, timestamp, proofs)
      } yield tx).fold(left => Failure(new Exception(left.toString)),
                       right => Success(right))
    }.flatten
  ...
  def create(version: Byte,
    ...
    for {
      ...
      new SetScriptTransaction(version,
                               networkByte,
                               sender,
                               script,
                               fee,
  ...
  def signed(version: Byte,
    ...
    create(version, sender, script, fee, timestamp, Proofs.empty).right.map {
      unsigned =>
        unsigned.copy(
          proofs = Proofs
