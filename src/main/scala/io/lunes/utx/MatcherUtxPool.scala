...
import io.lunes.state.{ByteStr, Diff, Portfolio}
import scorex.account.Address
import io.lunes.transaction.{AssetId, Authorized, Transaction, ValidationError}
import scorex.utils.ScorexLogging

import scala.collection.mutable

class MatcherUtxPool(underlying: UtxPool,
        ...
        case r @ (isNew, txDiff) =>
          if (isNew) {
            val msg = enrichWithAccountPortfolio {
              txDiff.portfolios.collect {
                case (addr, p) if addr.stringRepr != matcherSettings.account =>
                  (addr, p.pessimistic)
              }
            }
            if (!msg.isEmpty) {
  ...
  override def removeAll(txs: Traversable[Transaction]): Unit =
    underlying.removeAll(txs)

  override def accountPortfolio(addr: Address): Portfolio =
    underlying.accountPortfolio(addr)
  ...
  override def transactionById(transactionId: ByteStr): Option[Transaction] =
    underlying.transactionById(transactionId)
  ...
  override private[utx] def createBatchOps: UtxBatchOps =
    new BatchOpsImpl(underlying.createBatchOps)

  private class BatchOpsImpl(underlying: UtxBatchOps) extends UtxBatchOps {
    private val accountInfos: mutable.Map[Address, Portfolio] =
      mutable.Map.empty

    def messages: Seq[BalanceChanged] =
      accountInfos.toMap.grouped(3).map(enrichWithAccountPortfolio).toSeq
      ...
      txDiff.portfolios.foreach {
        case (addr, txPortfolio) =>
          if (addr.stringRepr != matcherSettings.account) {
            val prevPortfolio =
              accountInfos.getOrElse(addr, Monoid[Portfolio].empty)
            val overriddenPortfolio =
              Monoid.combine(prevPortfolio, txPortfolio.pessimistic)
  ...
  private def enrichWithAccountPortfolio(
      changes: Map[Address, Portfolio]): BalanceChanged = BalanceChanged {
    changes
      .flatMap {
        case (addr, portfolio) =>
          val changedAssets: Set[Option[AssetId]] = portfolio.assets.keySet
            .map(Some(_)) ++ {
