...
trait UtxPool {
   ...
   *
   * @param tx
   *   provides a [[io.lunes.transaction.Transaction]] input interface.
   * @return
   *   Returns Either a true or false value case the transaction is valid or a validation Error otherwise.
   */
   ...
   *
   * @param txs
   *   provide a interface for [[scala.collection.mutable.Traversable]] of [[io.lunes.transaction.Transaction]].
   */
   ...
   * @param addr
   *   Scorex [[scorex.account.Address]].
   * @return
   *   the [[io.lunes.state2.Portfolio]].
   */
   ...
   * @return
   *   returns a [[scala.collection.immutable.Seq]] of [[io.lunes.transaction.Transaction]].
   */
   ...
   *   provides a ByteStr
   * @return
   *   returns a Option for a [[io.lunes.transaction.Transaction]].
   */
   ...
   * @param rest
   *   [[io.lunes.mining.TwoDimensionalMiningConstraint]] input
   * @param sortInBlock
   *   True if Sort for Block.
   * @return
   *   Returns a Tuple of a [[scala.collection.immutable.Seq]] of [[io.lunes.transaction.Transaction]] and a
   *   [[io.lunes.mining.TwoDimensionalMiningConstraint]].
   */
  def packUnconfirmed(
    rest: TwoDimensionalMiningConstraint,
    sortInBlock: Boolean
  ): (Seq[Transaction], TwoDimensionalMiningConstraint)

  /**
   * Provides a interface for batched operations.
   * @param f
   *   Annonymous function for processing [[io.lunes.utx.UtxBatchOps]].
   */
...
trait UtxBatchOps {
   ...
   *   Input Transaction.
   * @return
   *   Returns Eiher Boolean (case Success) or ValidationError (case Failure).
   */
  def putIfNew(tx: Transaction): Either[ValidationError, Boolean]
}
 ...
 * @constructor
 *   Creates a Transaction Pool with Implicit Scheduler
 * @param time
 *   Sets Time o
 * @param stateReader
 *   Sets the StateReader object.
 * @param history
 *   Sets the Transaction History.
 * @param featureProvider
 *   Sets the FeatureProvider object.
 * @param feeCalculator
 *   Sets the FeeCalculator.
 * @param fs
 *   Sets the FuncionalitySettings.
 * @param utxSettings
...
class UtxPoolImpl(
   ...
   *   provides a [[io.lunes.transaction.Transaction]] input interface.
   * @return
   *   Returns Either a Boolean for the Transaction validity (case Success) or a
   *   [[io.lunes.transaction.ValidationError]] (case Failure).
   */
   ...
   *   Inputs Transaction
   * @return
   *   Returns Either a Unit (case Success) or a [[io.lunes.transaction.ValidationError]] (case Failure).
   */
   ...
   * Remove All Transactions in the Pool.
   * @param txs
   *   provide a interface for Transversable of [[io.lunes.transaction.Transaction]].
   */
  ...
  private def putIfNew(s: SnapshotStateReader, tx: Transaction): Either[ValidationError, Boolean] = {
    ...
    measureSuccessful(
      ...
      for {
        ...
        diff <-
          TransactionDiffer(fs, history.lastBlockTimestamp(), time.correctedTime(), s.height)(s, featureProvider, tx)
      } yield {
