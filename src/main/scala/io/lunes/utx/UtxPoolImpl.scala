...
import io.lunes.state.{Blockchain, ByteStr, Diff, Portfolio}
import kamon.Kamon
...
class UtxPoolImpl(time: Time,
  ...
  import io.lunes.utx.UtxPoolImpl._
  ...
  private val removeInvalid = Task {
    val b = blockchain
    val transactionsToRemove =
      transactions.values.asScala.filter(t => b.containsTransaction(t.id()))
  ...
  private val utxPoolSizeStats =
    Kamon.metrics.minMaxCounter("utx-pool-size", 500.millis)
  private val processingTimeStats = Kamon.metrics
    .histogram("utx-transaction-processing-time", KamonTime.Milliseconds)
  ...
  private def removeExpired(currentTs: Long): Unit = {
    def isExpired(tx: Transaction) =
      (currentTs - tx.timestamp).millis > utxSettings.maxTransactionAge
  ...
  override def putIfNew(
    ...
    putIfNew(blockchain, tx)

  private def checkNotBlacklisted(
  ...
  override def accountPortfolio(addr: Address): Portfolio =
    blockchain.portfolio(addr)

  override def portfolio(addr: Address): Portfolio =
    Monoid.combine(blockchain.portfolio(addr),
  ...
  override def packUnconfirmed(rest: MultiDimensionalMiningConstraint,
            ...
            differ(updatedBlockchain, tx) match {
              case Right(newDiff) =>
                ...
                if (updatedRest.isOverfilled)
                  (invalid, valid, diff, currRest, true)
                else
                  (invalid,
                   tx +: valid,
    ...
    val txs =
      if (sortInBlock) reversedValidTxs.sorted(TransactionsOrdering.InBlock)
      else reversedValidTxs.reverse
    (txs, finalConstraint)
  }

  override private[utx] def createBatchOps: UtxBatchOps =
    new BatchOpsImpl(blockchain)
  ...
  private def canReissue(b: Blockchain, tx: Transaction) = tx match {
    case r: ReissueTransaction
        if b.assetDescription(r.assetId).exists(!_.reissuable) =>
      Left(GenericError(s"Asset is not reissuable"))
    case _ => Right(())
  }

  private def checkAlias(b: Blockchain, tx: Transaction) = tx match {
    case cat: CreateAliasTransaction if !blockchain.canCreateAlias(cat.alias) =>
      Left(GenericError("Alias already claimed"))
    case _ => Right(())
  }

  private def putIfNew(
      b: Blockchain,
      tx: Transaction): Either[ValidationError, (Boolean, Diff)] = {
    ...
    def remove(txId: ByteStr): Unit = {
      if (Option(transactionPortfolios.remove(txId)).isDefined) {
        transactions.keySet().asScala.foreach { addr =>
          transactions.put(addr,
                           transactions.getOrDefault(addr, Set.empty) - txId)
        }
      }
    }
  }
