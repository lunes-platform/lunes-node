...
import io.lunes.transaction.{TransactionFactory, ValidationError}
...
case class AddressApiRoute(settings: RestAPISettings,
  ...
  private def addressScriptInfoJson(
      account: Address): Either[ValidationError, AddressScriptInfo] =
    for {
      script <- Right(blockchain.accountScript(account))
      complexity <- script.fold[Either[ValidationError, Long]](Right(0))(x =>
        ScriptCompiler.estimate(x).left.map(GenericError(_)))
    } yield
      AddressScriptInfo(
      ...
      .map(
        acc =>
          ToResponseMarshallable(
            Balance(acc.address,
                    confirmations,
                    blockchain.effectiveBalance(acc,
                                                blockchain.height,
                                                confirmations))))
      .getOrElse(InvalidAddress)
  }
    ...
    json[SignedMessage] { m =>
      if (Address.fromString(address).isLeft) {
        ...
        val msg: Try[Array[Byte]] =
          if (decode) Base58.decode(m.message) else Success(m.message.getBytes)
