...
object ApiError {
  def fromValidationError(e: ValidationError): ApiError = e match {
    ...
    case ValidationError.BannedAddress(x) => BannedAddress(x)
...
case class BannedAddress(msg: List[String]) extends ApiError {
  override val id: Int          = 666
  override val message: String  = s"🚨 Address ${msg} are banned."
  override val code: StatusCode = StatusCodes.BadRequest
}
