...
case class BlocksApiRoute(
  ...
  @ApiOperation(value = "At(Block header only)",
                notes =
  ...
  @ApiOperation(value = "Seq",
                notes = "Get block at specified heights",
                httpMethod = "GET")
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "from",
  ...
  @ApiOperation(value = "Seq (Block header only)",
                notes =
        ...
        .map {
          _.map(score =>
            allChannels.broadcast(
              LocalScoreChanged(score.getOrElse(blockchain.score))))
        }
