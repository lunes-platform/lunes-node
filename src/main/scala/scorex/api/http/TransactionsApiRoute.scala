...
case class TransactionsApiRoute(settings: RestAPISettings,
  ...
  private val invalidLimit = StatusCodes.BadRequest -> Json.obj(
            ...
            pathEndOrSingleSlash {
              ...
              path(Segment) { limitStr =>
                Exception.allCatch.opt(limitStr.toInt) match {
                  case Some(limit)
                      if limit > 0 && limit <= MaxTransactionsPerRequest =>
                    complete(
  ...
  @Path("/unconfirmed")
  @ApiOperation(value = "Unconfirmed",
  ...
  @ApiOperation(
    value = "Size of UTX pool",
    notes = "Get number of unconfirmed transactions in the UTX pool",
    httpMethod = "GET")
  ...
  @ApiOperation(value = "Transaction Info",
                notes = "Get transaction that is in the UTX",
                httpMethod = "GET")
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "id",
  ...
  @ApiImplicitParams(Array(
    new ApiImplicitParam(
      ...
      paramType = "body",
      value =
        "Transaction data including type and optional timestamp in milliseconds")
  ))
  ...
  @ApiOperation(value = "Sign a transaction by a private key of signer address",
                notes = "Sign a transaction",
                httpMethod = "POST")
  @ApiImplicitParams(
    Array(
  ...
  @Path("/broadcast")
  @ApiOperation(value = "Broadcasts a signed transaction",
                notes = "Broadcasts a signed transaction",
                httpMethod = "POST")
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "json",
                           value =
                             "Transaction data including type and signature",
                           required = true,
                           dataType = "string",
                           paramType = "body")
    ))
        ...
        val r = TransactionParsers.by(typeId, version) match {
          ...
          case Some(x) =>
            x match {
              ...
              case MassTransferTransaction =>
                jsv.as[SignedMassTransferRequest].toTx
  ...
  private def txToExtendedJson(tx: Transaction): JsObject = {
    ...
    tx match {
      case lease: LeaseTransaction =>
        import io.lunes.transaction.lease.LeaseTransaction.Status._
        lease.json() ++ Json.obj(
