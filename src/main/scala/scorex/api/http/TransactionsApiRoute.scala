...
case class TransactionsApiRoute(
        ...
        ByteStr.decodeBase58(encoded) match {
          case Success(id) =>
            state().transactionInfo(id) match {
              ...
              case None =>
                complete(
                  StatusCodes.NotFound -> Json.obj("status" -> "error", "details" -> "Transaction is not in blockchain")
                )
            }
        ...
        ByteStr.decodeBase58(encoded) match {
          case Success(id) =>
            utx.transactionById(id) match {
              ...
              case None =>
                complete(
                  StatusCodes.NotFound -> Json.obj("status" -> "error", "details" -> "Transaction is not in UTX")
                )
            }
          case _ => complete(InvalidSignature)
        }
      ...
      json[JsObject] { jsv =>
        ...
        val txEi = TransactionType((jsv \ "type").as[Int]) match {
          ...
          case MassTransferTransaction =>
            TransactionFactory.massTransferAsset(jsv.as[MassTransferRequest], wallet, time)
  ...
  @Path("/broadcast")
  @ApiOperation(
    value = "Broadcasts a signed transaction",
    ...
    tx match {
      ...
      case leaseCancel: LeaseCancelTransaction =>
        leaseCancel.json() ++ Json.obj(
          "lease" -> state()
            .findTransaction[LeaseTransaction](leaseCancel.leaseId)
            .map(_.json())
  ...
  /**
   * Produces compact representation for large transactions by stripping unnecessary data. Currently implemented for
   * MassTransfer transaction only.
   */
