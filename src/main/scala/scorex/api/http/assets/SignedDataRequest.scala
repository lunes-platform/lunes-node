...
import io.lunes.transaction.assets.DataTransaction
...
case class SignedDataRequest(@ApiModelProperty(value = "Base58 encoded sender public key", required = true)
  ...
  def toTx: Either[ValidationError, DataTransaction] = for {
    ...
    _userdata <- parseBase58(userdata.filter(_.length > 0), "invalid.userdata", DataTransaction.MaxUserdataLength)
    _account <- AddressOrAlias.fromString(recipient)
    t <- DataTransaction.create(_assetId, _sender, _account, amount, timestamp, _feeAssetId, fee, _userdata.arr, _signature)
  } yield t
}
